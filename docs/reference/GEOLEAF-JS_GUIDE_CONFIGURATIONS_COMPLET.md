...
| --- | ---------- | ------------------------- | ---------------- | ------------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `debug`    | —                         | boolean          | `false`      | Mode debug global                                    | Active un mode verbeux qui affiche dans la console des logs détaillés pour toutes les opérations GeoLeaf (chargement de couches, rendu de POI, application de filtres, etc.). Très utile lors de l'intégration. Il fait aussi DEUX choses qui ne se devinent pas : toute requête de configuration part avec un `?t=<horloge>`, ce qui défait le cache du navigateur ET celui du service worker à chaque chargement ; et le bundle de profil pré-construit est ignoré au profit des fichiers de section. ⚠️ Ce n'est donc PAS la réponse à un profil périmé en production — voir `data.profileVersion` (#7). À désactiver impérativement en production pour ne pas polluer la console ni dégrader les performances.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 7   | `data`     | `profileVersion`          | string \| number | `0`          | Empreinte de contenu du profil                       | Jeton ajouté en `?t=` à **chaque** requête de ressource de profil : `profile.json`, `mapping.json`, les fichiers de section, les configs de couche et le bundle. ⚠️ **Opaque, et jamais une date** — un hash, un compteur de révision, un identifiant de déploiement : la valeur n'est ni lue ni comparée, seul importe qu'elle change **quand le contenu change**. Une horloge défait le mécanisme, puisqu'elle fait re-télécharger à chaque chargement ce qui n'a pas bougé. **Dans un dossier livré (`deploy/`), vous n'avez rien à écrire : le build la pose**, sous la forme d'une empreinte du contenu de `profiles/`. Elle ne concerne que l'intégrateur qui construit sa configuration lui-même, seul à savoir quand son profil a changé — et l'écrire dans `profile.json` est trop tard, le jeton est résolu avant que ce fichier soit requêté. 🛑 **Servez `profiles/geoleaf.config.json` en `no-cache`** : c'est le fichier qui porte l'empreinte, et on ne peut pas invalider par un jeton le fichier qui porte ce jeton — épinglé, il défait tout le mécanisme. Les recettes `nginx.conf.example` et `.htaccess` livrées avec le dossier posent déjà la règle. **Absent, le jeton vaut `0`, donc une URL fixe** : si votre serveur envoie un `Cache-Control: max-age` long sur ses fichiers statiques, le profil et toutes ses annexes restent épinglés dans un navigateur déjà ouvert pour toute la durée de cet en-tête — ni un vidage de cache serveur ni une régénération n'y changent rien. |
...
| 23  | `ui.showCacheButton`             | boolean | `false`  | Bouton cache offline        | Affiche un bouton permettant à l'utilisateur de gérer le cache hors-ligne (téléchargement des données et tuiles pour utilisation sans réseau). Ce bouton ouvre une modale avec les options de cache, la progression des téléchargements et les statistiques de stockage. **Requiert le plugin Storage** — si le plugin n'est pas chargé, le bouton ne fait rien et un message d'avertissement est affiché dans la console. ⚠️ Depuis le 24/08/2026, cette clé ne gouverne QUE la visibilité du bouton : elle ne coupe plus la capture de carte du module cache (`modules.offline-ui.showButton` est la clé canonique, celle-ci reste le repli). |
...
> **Deux engagements du moteur de thèmes, garantis — ils ne sont pas des détails d'implémentation.**
>
> **① Déclarer `themes` directement dans le profil supprime toute requête HTTP.** Si le profil
> actif porte déjà un objet `themes`, GeoLeaf le lit tel quel : **aucun `fetch` n'est émis** et le
> chemin `Files.themesFile` n'est pas consulté. C'est la voie à suivre quand vos assets ne
> suivent pas la convention de chemin de GeoLeaf — **vous n'avez pas à intercepter
> `window.fetch`** pour servir `themes.json`. La garantie est contractuelle depuis la 3.1.0 :
> réintroduire une requête sur ce chemin serait une rupture majeure, pas un réglage.
>
> **② `GeoLeaf.Config.clearThemesCache(profileId?)` vide ce cache** — un profil, ou tous si
> l'argument est omis, promesses de chargement en cours comprises. Utile après avoir remplacé
> les thèmes à chaud.
>
> ```js
> GeoLeaf.Config.clearThemesCache("mon-profil");
> ```
>
> ⚠️ Ne pas confondre avec `GeoLeaf.ThemeCache`, qui est le cache IndexedDB des **données de
> couche** — un homonyme, sans rapport avec `themes.json`.
...
| 65e | `data.ogcApi.maxFeatures`         | number  | `10000`   | Limite de features               | Nombre maximum de features chargées, **toutes pages confondues** — à ne pas confondre avec le paramètre `limit` de la requête OGC, qui est la taille d'UNE page (défaut 1000, clé `data.ogcApi.limit`). Limite anti-DoS mémoire : une collection trop grande peut saturer le navigateur. ⚠️ Le plafond **tronque** : au-delà, la collection porte un membre `truncated` et un avertissement est affiché une fois par couche. ⚠️ Ce guide annonçait `5000` et décrivait la clé comme « le paramètre `limit` de l'API » — les deux étaient faux, corrigés par R9 sur `kernel/geojson/loader/ogc-api-loader.ts`. |
...
> **⚠️ `label.offset.placement` nomme le côté où l'on VOIT l'étiquette, pas l'ancre MapLibre.**
> Les deux vocabulaires sont opposés : dans MapLibre, `text-anchor: "top"` colle le _haut du
> texte_ au point, donc le texte descend. Le paramètre décrit ici est le côté du **label** —
> `"top"` met bien l'étiquette au-dessus de l'entité. La conversion est faite par le moteur de
> rendu, il n'y a rien à inverser dans le profil.
...
| 132  | `label.offset.distancePx`  | number  | `12`       | Écart entre l'entité et le label | Distance en pixels entre l'entité et le **bord le plus proche de la boîte de texte** — c'est donc l'espace blanc réellement visible, pas une distance à son centre. Ignorée quand `placement` vaut `center`. Ordre de grandeur utile : le rayon du marqueur plus 3 px, ou la moitié de la hauteur affichée de l'icône (une icône de 64 px rendue à `icon-size` 0.5 occupe 32 px, donc environ 16 px de demi-hauteur). En dessous d'environ un tiers de la taille du texte, la correction typographique appliquée par le moteur annule le décalage : l'étiquette se recolle à l'entité au lieu de s'en écarter.                                              |
| 132a | `label.offset.placement`   | string  | `"center"` | Côté où se pose le label         | Côté sur lequel l'étiquette est dessinée par rapport à l'entité. Neuf valeurs : `center` (défaut, comportement historique — le texte se pose sur l'entité), les quatre cardinales `top`, `bottom`, `left`, `right`, et les quatre diagonales `top-left`, `top-right`, `bottom-left`, `bottom-right`. Les diagonales écartent d'autant sur les deux axes que les cardinales sur un seul : la distance est mesurée de la même façon dans les huit directions. Tant que la valeur est `center`, ou que `distancePx` vaut 0, **aucune propriété de placement n'est envoyée au moteur** — le rendu est strictement celui d'avant l'introduction de ce paramètre. |
...
| ---------------- | -------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `actionId`       | string   | ✅     | **honoré** — identifiant opaque de l'action (ex. `"host:open-form"`), transmis dans l'événement. **Absent, vide ou non-string, le bouton n'est pas rendu du tout.**                                                                                |
...
```json
{
    "popup": {
        "fields": [
            ...
            {
                "type": "action",
                "actionId": "host:open-form",
                "label": "Ouvrir la fiche",
                "confirm": "Ouvrir la fiche ?",
...
Un seul mécanisme : l'écoute de l'événement `geoleaf:popup:action`, faiblement couplé. Le bouton rend sur la bulle **et** sur le panneau latéral — les deux surfaces partagent une table de rendu.

⚠️ **Depuis le 14/08/2026, le payload n'est PLUS sérialisable.** Il porte `button` (le nœud cliqué), `setBusy(busy)` et `close()` en plus des champs de données : `JSON.stringify(e.detail)` **jette**, et le passer à `postMessage` jette `DataCloneError`. Recopier les champs utiles. Et `properties` est **vide tant que le bouton ne déclare pas `payloadFields`** — le défaut va vers la confidentialité, cet événement étant écoutable par n'importe quel script de la page.
...
GeoLeaf.events.on("geoleaf:popup:action", async (e) => {
    ...
    const res = await fetch("/api/poi/open", {
        method: "POST",
...
pas transposable, au lieu d'afficher des tuiles décalées sans rien dire :
...
| `proximity` | `radiusMin` / `radiusMax` / `radiusStep` / `radiusDefault` (**en km, décimales acceptées**), `buttonLabel?`, `instructionText?` | curseur rayon + point carte      | distance haversine ≤ rayon                                                |
...
Détail : [`specs/capacites/filter.md`](../specs/capacites/filter.md).

⚠️ **Les quatre `radius*` sont en KILOMÈTRES et acceptent des décimales.** `radiusStep: 0.1`
donne une granularité de 100 m, `radiusDefault: 0.5` un rayon initial de 500 m. Ce n'était pas
vrai avant le 20/08/2026 : la barre de proximité lisait son curseur au `parseInt`, si bien que
tout rayon sous 1 km s'écrasait à **zéro** — cercle invisible, filtre ne gardant rien — et que
1,4 km devenait 1,0 km, **le libellé continuant d'afficher la valeur demandée**. Les bornes
étaient pourtant lues correctement : c'est ce qui en était fait ensuite qui les perdait. Le
défaut in-core reste `{ min: 1, max: 50, step: 1, default: 10 }`, dimensionné pour une carte
touristique ; un profil qui a besoin d'une autre granularité les surcharge, il n'a pas à
s'aligner sur des kilomètres entiers.
...
| 254a | `modules.editor.showAddPoi`                   | boolean         | `true`             | Bouton « ajouter un POI » | Affiche le bouton de capture de POI dans la barre pill mobile. ⚠️ **Remplace `ui.showAddPoi`**, que le CORE lisait pour décider d'un bouton que seul un plugin pouvait servir. 🛑 **Le défaut CHANGE de sens** : `ui.showAddPoi` valait `false` (opt-in), celui-ci vaut `true` (opt-out), comme les autres créneaux paresseux du plugin. |
...
🛑 **`@geoleaf-plugins/addpoi` a fusionné dans `@geoleaf-plugins/editor` le 05/08/2026.** Le bloc
...
| ---- | -------------------------------- | ------- | -------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 270  | `ui.theme`                       | string  | `"auto"` | Thème visuel                    | Définit l'apparence de tous les composants d'interface : `"light"` (fond clair), `"dark"` (fond sombre) ou `"auto"` (suit `prefers-color-scheme`). Panneaux, popups, notifications, boutons. L'utilisateur peut basculer via la bascule du bandeau, tant que `ui.showPanelThemeToggle` ne vaut pas `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 282a | `ui.showPanelThemeToggle`        | boolean | `true`   | Bascule clair/sombre du bandeau | Affiche la bascule clair/sombre **possédée par le panneau desktop** : le bouton en bas du bandeau d'onglets (≥ 1440 px) et sa copie dans la barre d'outils mobile — un seul réglage pour les deux. Mettre `false` retire le bouton sans changer le thème. ⚠️ Ne gouverne pas `modules.theme-toggle` (le bouton posé **sur la carte**), ni un bouton `[data-gl-role="theme-toggle"]` fourni par l'intégrateur, ni `ui.theme`. 🛑 Masquer le bouton **ne reprend pas la main sur le thème** : le choix déjà mémorisé par un visiteur garde la priorité sur `ui.theme`, et si `modules.theme-toggle` est aussi désactivé il ne lui reste aucun déclencheur d'interface pour revenir (échappatoire : `localStorage.removeItem("geoleaf_theme")` en console). |
...
| 283g | `modules.geocoding.position` | string | `"top-left"` | Position sur la carte | Position du contrôle de géocodage : `"top-left"`, `"top-right"`, `"bottom-left"`, `"bottom-right"`. Défaut changé de `top-right` → `top-left` pour éviter le chevauchement avec le menu thème secondaire. |
...
- ~~La capacité est **exclue du build Lite**.~~ ⚠️ **Le build Lite N'EXISTE PLUS** (mesuré le 17/08/2026) — `rollup.config.mjs:650` : « The frozen "lite" build (S4, presets chantier) is GONE. It was never served ». Aucun script npm ne le nomme, et `boot-lite.ts` est absent du dépôt.
...
| `data.ogcApi.maxFeatures`  | number  | `10000` | Plafond d'entités toutes pages confondues ; au-delà, la collection est tronquée et signalée |
...
⚠️ **Quatre réglages du dépôt portent le mot « thème » et sont indépendants** (ils se cumulent) :

| Réglage                   | Ce qu'il change                                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------- |
| `modules.theme-palette`   | La **couleur d'accent** de l'interface (cette section)                                    |
| `modules.theme-toggle`    | Le bouton clair/sombre **posé sur la carte** (contrôle MapLibre ; opt-in, défaut **OFF**) |
| `ui.showPanelThemeToggle` | Le bouton clair/sombre **du bandeau d'onglets** et sa copie mobile (opt-out, défaut ON)   |
| `modules.theme-selector`  | Les **thèmes de CARTE** (jeux de couches, `themes.json`)                                  |

💡 Les deux lignes du milieu sont **deux boutons distincts** pour la même bascule. Celui que
l'application affiche par défaut est le second (`ui.showPanelThemeToggle`) ; `modules.theme-toggle`
est un contrôle carte séparé, éteint tant qu'un profil ne l'allume pas. Poser
`modules.theme-toggle.enabled: false` pour faire disparaître le bouton visible **ne fait rien** :
c'est déjà son défaut.
