...
**Code :** `packages/plugins/geocoding/` · **Vérifié contre :** `5535694b` (27/07/2026)
...
| 5 — créneau de barre d'outils      | `registry.register({ id, ui: { mobileIcon: … } })` — **mobile uniquement**                |
...
⚠️ **La `profileKey` du créneau est `ui.showGeocoding`**, une clé **hors** de `modules.geocoding`.
C'est la clé historique du bouton de recherche du core : le créneau de barre d'outils et la
...
| GC-16 | Pastille mobile                                | Action de barre d'outils `geocoding`          | La pastille de recherche s'ouvre et le champ prend le focus                                                                                                                        | `entry.ts` étape 6 ; `ui/pill-search.ts` |
...
| `position`     | l'un des quatre coins                    | coin supérieur droit             | Position du contrôle                                                  |
...
| Dépendance              | Nature                                                                                                                             |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `@geoleaf/core`         | Dépendance d'espace de travail ; chargé **après** le core, **avant** `boot()`. Consommé **en type** pour le seam de barre d'outils |
| `@geoleaf/host-runtime` | Consommé **fonctionnellement** — `getGeoLeaf()` et `coreConfigGet()`, pas seulement des types                                      |
...
**Aucune dépendance sur MapLibre.** PC-10 ne s'applique pas.
