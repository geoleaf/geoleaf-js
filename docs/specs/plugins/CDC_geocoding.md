...
**Code :** `packages/plugins/geocoding/` · **Vérifié contre :** voir `verifie_contre` en tête — une seconde empreinte vivait ici, que rien ne gardait ; cf. `__tests__/guards/spec-single-stamp.guard.test.ts`.
...
Il pose une **barre de recherche** sur la carte, interroge un ou plusieurs fournisseurs, propose les
résultats, et amène la carte sur celui que l'utilisateur choisit. La recherche est aussi disponible
**par programme**, sans interface.

Les fournisseurs se résolvent par un **registre** (1.1.0 — un `switch` fermé jusque-là) : la base
d'adresses nationale française, deux services mondiaux, **n'importe quel point d'accès HTTPS**
rendant une collection d'entités GeoJSON, **`layers` — les entités que la carte tient, sans réseau**,
par `GeoLeaf.Layers.search` —, et ce qu'un intégrateur ajoute par `registerProvider`. Le patron est
celui du plugin [`routing`](CDC_routing.md), qui refusait déjà toute substitution silencieuse.
...
- **Il n'ajoute pas de couche** : il déplace la vue, il ne matérialise pas le résultat en entité. Une
  entité trouvée par `layers` est sélectionnée par le core (`GeoLeaf.Layers.focus`), pas par lui.
- **Il ne cherche pas dans une couche non chargée, ni en tuiles vectorielles** : `layers` lit ce que
  la carte tient — c'est le contrat de `GeoLeaf.Layers.search`.
...
| GC-05 | Choix d'un résultat                            | Clic ou validation au clavier                           | Une ENTITÉ trouvée dans la carte est **cadrée et sélectionnée** (`GeoLeaf.Layers.focus`) ; une adresse — ou une entité que le core ne sait pas focaliser — fait **voler** la carte vers un point, ou **cadrer** une emprise ; puis un événement est émis                                                                                   | `registry.ts` → `_onSelect`                                           |
...
| GC-11 | **Valeur de fournisseur inconnue ou non sûre** | `provider: "nominatum"`, `"http://…"`, `"javascript:…"` | **Seule** : repli sur le fournisseur par défaut, désormais **NOMMÉ** par un avertissement qui annonce le refus à la mineure suivante (1.1.0) — le repli envoie la saisie à ce service, c'est pourquoi il ne pouvait pas rester muet. **Dans une liste** : refusée, rien n'est interrogé pour elle. Jamais d'appel vers la valeur elle-même | `provider-registry.ts` → `createProvider`                             |
...
| GC-21 | **Les entités de la carte, sans réseau**       | `provider: "layers"` (seul ou dans une liste)           | `GeoLeaf.Layers.search` interrogé : les entités des couches `searchable`, **sans une requête** ; un core sans ce membre rend une liste vide, sans lever                                                                                                                                                                                    | `provider-registry.ts` → fournisseur `layers`                         |
| GC-22 | **Liste ordonnée, hors réseau d'abord**        | `provider: ["layers", "nominatim"]`                     | Résultats dans l'ordre de la liste, dans la limite ; **navigateur hors ligne : les services réseau ne sont pas interrogés** ; un fournisseur qui échoue rend vide sans emporter les autres                                                                                                                                                 | `provider-registry.ts` → liste                                        |
| GC-23 | Registre de fournisseurs                       | `registerProvider(name, factory)`                       | Le nom devient utilisable dans `provider` ; résolu **à chaque recherche**, donc pris en compte après le montage                                                                                                                                                                                                                            | `provider-registry.ts`, `registry.ts`                                 |
| GC-24 | **Aucun résultat, dit**                        | Recherche sans réponse                                  | « Aucun résultat » affiché (`role="status"`), effacé à la frappe suivante — une liste vide se lisait comme une panne                                                                                                                                                                                                                       | `control.ts`                                                          |

Les tests qui couvrent ces lignes : `packages/plugins/geocoding/src/__tests__/`, dont un **contrat de
configuration** qui vérifie clé par clé la traduction vers les URL des fournisseurs (PC-09), et le
registre de fournisseurs (`provider-registry.test.ts`). Preuve sur le bundle livré, hors réseau :
`e2e/59-local-search-offline.spec.js`.
...
| `registerProvider(name, factory)` | Rend un fournisseur utilisable sous ce nom dans `provider` (1.1.0) ; lève sur un nom vide ou une fabrique qui n'en est pas une |
| `destroy()`                       | Démonte le contrôle et relâche les écouteurs                                                                                   |

La façade porte des `@example` — une recherche par programme, l'écoute de l'événement de sélection,
...
| **Valeur non reconnue : repli NOMMÉ, puis refus**                | Le repli appelait un service tiers avec la saisie de l'utilisateur, sans un mot : 1.1.0 le nomme et annonce son retrait, la mineure suivante le retire (`packages/core/docs/VERSIONING_POLICY.md` : un comportement ne se retire qu'après une version publiée qui l'annonce). Dans la forme liste, neuve, rien ne s'appuie sur un repli : refus immédiat | Garder le repli muet, ou refuser dès 1.1.0 sans annonce                                            |
| **Registre de fournisseurs plutôt que `switch`**                 | Un fournisseur local ou métier devient une ligne d'enregistrement, pas une modification du plugin ; résolu à chaque recherche, il est vu même enregistré après le montage                                                                                                                                                                                | Ajouter une branche au `switch` par fournisseur                                                    |
| **La recherche d'entités vit dans le core, le plugin l'expose**  | La donnée (le magasin des couches, relu du stockage de l'appareil) et la sélection vivent dans le core ; une tournée ou un intégrateur l'appellent sans ce plugin ; `no-plugin-in-core` interdit le sens inverse                                                                                                                                         | Un fournisseur qui relirait les couches lui-même ; une capacité du core avec sa propre barre       |
...
| `GeoLeaf.Layers` (core ≥ 3.10.0) | Lu **au moment de l'appel** par le fournisseur `layers` (`search`) et par la sélection (`focus`) ; un core plus ancien rend une liste vide et fait voler la carte, sans lever                                                                                                                                                                            |
