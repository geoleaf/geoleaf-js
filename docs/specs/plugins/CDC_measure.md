...
Ses deux champs de dépendance sont **exacts** : `requires: []` et `optional: []`, comme `entry.ts`
les déclare.

⚠️ **Cette phrase disait « le seul des quatre manifestes de ce lot », en opposant `table`, `print` et
`addpoi`.** `addpoi` **n'existe plus** — fusionné dans `editor` au S5 —, donc la comparaison a perdu
son quatrième terme, et un lecteur n'a aucun moyen de la vérifier. L'exactitude de chaque manifeste
se constate désormais fiche par fiche, par sa propre garde
(`doc-plugin-manifest.guard.test.js`), pas par un classement recopié ici.
...
| ME-02 | Mesure de distance en polyligne                              | Clics successifs                                                     | Longueur cumulée, segment par segment, mise à jour en continu. **Trois gestes de fermeture** : double-clic, barre d'espace, ou **re-clic sur le dernier sommet** dès deux sommets. ⚠️ Le troisième est ce qui rend la mesure finissable **au doigt** — un double-tap ne synthétise pas de `dblclick` ici, et ces taps tombaient jusqu'au 25/08/2026 dans la garde anti-doublon, qui les avalait : au tactile, la distance ne pouvait pas se terminer. Preuve navigateur : `e2e/34-measure-close.touch.spec.js` | `tools/tool-distance.ts`, `compute.ts`     |
...
| ME-08 | Fermeture PROPOSÉE de la trace GPS                           | Retour près du point de départ                                       | Dès trois sommets, si la position revient sous le seuil, une **modale demande** de fermer en polygone — elle ne ferme rien d'elle-même. « Oui » ferme **et arrête** l'outil ; « Non » la ré-arme, de sorte qu'un second passage près du départ la reproposera                                                                                                                                                                                                                                                  | `tools/tool-gps.ts`                        |
| ME-09 | Annotation libre, UNE forme                                  | Outil d'annotation                                                   | Une bulle redimensionnable et éditable, ancrée à une coordonnée. ⚠️ L'« étiquette simple » n'existe plus : `createOverlay` pose toujours `annotationKind: "tooltip"`, et la relecture du stockage **migre** en `"tooltip"` toute entité `"label"` d'un profil ancien. Le texte n'est PAS plafonné — voir `labelMaxChars` au §Configuration                                                                                                                                                                     | `tools/tool-annotation.ts`                 |
...
| ME-12 | Persistance locale                                           | `persist`                                                            | Les mesures survivent au rechargement, sous une clé de stockage configurable. L'écriture est différée (~300 ms), et **une écriture en attente est vidée à `pagehide` et au passage de la page en arrière-plan** : sans ce vidage, une mesure terminée juste avant un rechargement était perdue — sous charge, le délai réel dépassait 600 ms                                                                                                                                                                   | `persistence.ts`                           |
| ME-13 | Plafond du nombre de mesures                                 | `maxFeatures`                                                        | 🛑 **Rien n'est tronqué.** Au-delà du plafond, `scheduleSave` **abandonne l'écriture** en avertissant — la sauvegarde précédente reste donc en place, **plus ancienne que l'écran** — et `measure-api.ts` **refuse toute annotation neuve**. Les mesures déjà dessinées ne sont ni supprimées ni bornées                                                                                                                                                                                                       | `config.ts`, `persistence.ts`              |
| ME-14 | Export de la collection                                      | Action d'export                                                      | Un fichier géographique, dont le nom est configurable                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `geojson-export.ts`                        |
| ME-15 | Accrochage au PREMIER sommet, pour FERMER                    | `snapPx`                                                             | À partir de trois sommets, quand le pointeur revient dans un rayon en pixels du **premier** sommet, le curseur passe à `pointer` et un clic **ferme l'anneau**. Rien ne s'aimante : aucun point n'est déplacé, c'est la fermeture qui est offerte — et le **polygone est le seul outil concerné**                                                                                                                                                                                                              | `tools/tool-polygon.ts`                    |
...
| ME-22 | **Les mesures traversent un changement de fond**             | Basculement vers ou depuis un fond vectoriel — le style est remplacé | Les sources et couches `gl-measure-*` sont portées dans le style entrant, avec les mesures qu'elles contiennent : `initLayers` les déclare à l'adaptateur qui pilote la carte (`declareOwnedStyleIds`, core ≥ 3.10.0). ⚠️ Posées hors du registre de l'adaptateur, elles étaient effacées par le diff, et chaque mesure terminée avec elles. Un cœur plus ancien : comportement d'avant, sans erreur                                                                                                           |
...
| `position`            | `string`  | `"left"`                     | 🛑 **CLÉ MORTE** — déclarée dans `types.ts`, défaultée dans `config.ts`, **lue nulle part**. L'ancrage du bouton est décidé par le créneau de barre d'outils, jamais par elle                                       |
...
| `labelMaxChars`       | `number`  | `120`                        | 🛑 **CLÉ MORTE** — même cas que `position` : `grep -rn labelMaxChars packages/plugins/measure/src` ne rend que sa déclaration (`types.ts`) et son défaut (`config.ts`). **Aucun texte d'annotation n'est plafonné** |
...
⚠️ **Ce plugin VALIDE sa configuration**, au lieu de simplement fusionner sur des défauts : les deux
émunérations d'unités sont vérifiées, `enabledTools` est **filtré** contre la liste des outils
connus, une position de menu invalide retombe sur le défaut **avec un avertissement**, et un jeu de
valeurs numériques est borné.

⚠️ **Cette ligne a écrit « sept valeurs numériques sont bornées » — un chiffre recopié dans une fiche
qui interdit d'en recopier un, et qui était déjà faux à la vérification.** Le compte se lit à la
commande, jamais ici :

```bash
grep -n 'Math.max\|Math.min' packages/plugins/measure/src/config.ts
```

⚠️ **`showButton` a un repli sur `ui.showMeasure`**, l'ancienne clé racine — c'est le seul reste de
migration lisible dans ce plugin, et il est **délibéré**.

⚠️ **Cette ligne a dit « la `profileKey` du créneau, elle, pointe toujours `ui.showMeasure` », et
c'est l'inverse depuis le 17/08/2026** : le créneau déclare
`profileKey: "modules.measure.showButton"` et garde `ui.showMeasure` en **`legacyProfileKey`** —
un repli qui ne parle que dans le silence de la clé canonique (`ui-slot-builder.ts`,
`_configuredVisibility`). La lecture de `config.ts` reflète délibérément la même précédence.
Les deux clés cohabitent donc **des deux côtés**, la clé de module prioritaire dans les deux, et il
n'y a plus de « reste de migration » asymétrique à signaler.
...
**Un émis** — `geoleaf:submenu:open`, `detail.id = "measure"`, à chaque ouverture du menu flottant
(`floating-menu.ts`, `_open()`). C'est le protocole d'**exclusion mutuelle des sous-menus** : le
plugin l'écoute aussi et **ferme le sien** dès qu'un autre `id` l'émet. ⚠️ La fiche a écrit « aucun
émis » ; l'émission était déjà là au moment de la vérification — l'énoncé était faux à sa date.

**Trois écoutés :**

| Événement                         | Où                  | Pourquoi                                                                                                     |
| --------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------ |
| `geoleaf:toolbar:action`          | `entry.ts`          | filtré sur `measure` — ouvre le menu, en transportant l'élément déclencheur                                  |
| `geoleaf:submenu:open`            | `floating-menu.ts`  | ferme ce menu quand un autre sous-menu s'ouvre                                                               |
| `geoleaf:geolocation:statechange` | `tools/tool-gps.ts` | sur le conteneur de carte, outil GPS armé : coupe la mesure si la géolocalisation est arrêtée de l'extérieur |

⚠️ **Le dernier s'appelait `gl:geoloc:statechange` jusqu'au 16/08/2026** (EM-03 : un littéral
d'événement qui contient un `:` doit commencer par `geoleaf:`). Il est émis par le core avec
`bubbles: true` et écouté ici : il **franchit la frontière core → plugin**, c'est donc un contrat
public quel que soit le nom qu'il porte. Le recensement se refait, il ne se recopie pas :

```bash
grep -rn 'geoleaf:' packages/plugins/measure/src --include=*.ts | grep -v __tests__
```
...
| **Fermeture PROPOSÉE, jamais automatique, sur la boucle GPS** | Mesurer une parcelle en la longeant est le cas d'usage terrain, et exiger un clic de fermeture **exact** au retour serait impraticable — mais fermer sans demander confisquerait une trace que l'opérateur voulait peut-être poursuivre. Le compromis retenu est une modale au passage près du départ (`_showCloseModal`), refusable et re-proposée au tour suivant                                                                                                                                                                                                                                                                                                                                                           | Une fermeture manuelle                  |
...
| **Tout outil armé porte un garde-fou de curseur**             | **Cinq** des six outils posent `crosshair` sur le canvas — distance, polygone, annotation, cercle et rectangle. ⚠️ **L'outil GPS n'écrit AUCUN curseur** : `tools/tool-gps.ts` ne contient pas un seul `setCursor`, et n'a donc pas de garde-fou non plus — il ne se pilote pas au pointeur, la trace vient de `watchPosition`. La règle porte sur les outils qui dessinent au pointeur, pas sur « les six ». D'autres gestionnaires écrivent le même style au survol des entités. Les outils au clic avaient un `MutationObserver` depuis le S5 ; `createDragTool` — donc **cercle et rectangle** — n'en avait aucun, et perdait sa croix au premier `mouseleave` de POI, définitivement. Mesuré au navigateur le 14/08/2026 | Compter sur le seul `setCursor` initial |
...
🛑 **Frontière avec `@geoleaf/host-runtime` : `src/internal.ts`, et ses trois enveloppes sont un
CONTRAT DE PUBLICATION, pas du style.** Le paquet emprunte cinq primitives partagées
(`getNativeMap`, `warnNoCore`, `createEl`, `applyStyleText`, `tLabel`) sous le vocabulaire local du
plugin. Trois d'entre elles sont des **fonctions locales à signature écrite**, jamais des
ré-exports : un `export { createEl as _el } from "@geoleaf/host-runtime"` fait émettre par `tsc` le
nom du paquet dans `dist/types/internal.d.ts` — or `host-runtime` est `private: true`, donc **404
sur npm pour toujours**. Vert ici par le lien de workspace, `TS2307` chez **tout** intégrateur.
⚠️ Dériver les signatures par `typeof createEl` ré-introduit la même fuite avec **moins** de
visibilité. Gardé par `scripts/check-shipped-specifiers.cjs` (SHIP-SPEC). Les autres imports de
`host-runtime` — `getGeoLeaf`, les utilitaires de menu, `downloadBlob`, `coreConfigGet` et, depuis la
1.0.6, `declareOwnedStyleIds` dans `draw-layers.ts` (ME-22) — sont DIRECTS : consommés dans le
corps, jamais ré-exportés, ils ne nomment pas le paquet dans les `.d.ts` émis.
