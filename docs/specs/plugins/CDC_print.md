...
**Code :** `packages/plugins/print/` · **Vérifié contre :** voir `verifie_contre` en tête — une seconde empreinte vivait ici, que rien ne gardait ; cf. `__tests__/guards/spec-single-stamp.guard.test.ts`.
...
| PR-16  | **Le rendu hors écran demande ses ressources à la carte VIVANTE** | Transformation de requête de la carte affichée (celle d'un connecteur, ou le `transformRequest` de l'intégrateur) | Chaque requête du rendu hors écran passe par la transformation de la carte affichée, lue AU MOMENT de la requête : une tuile protégée s'imprime avec les en-têtes de l'écran. 🛑 **Jusqu'au 18/09/2026, elle partait sans aucune transformation** : le plugin lisait, derrière un transtypage écrit à la main, un champ que MapLibre n'écrit pas — il nomme le sien `_transformRequestFn` (`grep -n '_transformRequestFn' node_modules/maplibre-gl/dist/maplibre-gl.d.ts`). Une couche protégée s'imprimait vide             | `internal.ts` (`_liveTransformRequest`)                |

Les tests qui couvrent ces lignes : `packages/plugins/print/src/__tests__/`, plus les parcours
navigateur `e2e/14-print.spec.js` et, pour PR-16, `e2e/50-print-authenticated-tiles.spec.js`.
...
| **Le rendu hors écran DÉLÈGUE à la transformation de la carte vivante, il ne la copie pas** | Une copie prise à la capture fige la transformation du moment : une transformation remplacée pendant que la fenêtre d'impression reste ouverte — un connecteur reconfiguré, par exemple — ne l'atteindrait plus. Déléguer au gestionnaire de requêtes de la carte (`Map._requestManager`, déclaré par MapLibre) la lit à chaque requête, par des déclarations que le compilateur vérifie : renommer le membre casse la compilation du dépôt dès qu'il passe à cette version, au lieu de vider la planche                                         | Copier le champ privé de la transformation                                                 |
