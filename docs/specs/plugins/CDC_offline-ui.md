...
| OU-16 | **« Puis-je partir ? » dans la modale** (1.6.0)      | Ouverture de la fenêtre hors ligne                       | Sous le statut de synchronisation : le verdict du core (`Storage.preflight()`, core ≥ 3.10.0), le régime de persistance que le navigateur accorde, chaque couche qui déclare une source avec son état — **en alerte** quand elle n'est pas sur l'appareil —, et les zooms qu'une préparation a laissés de côté. Relu une fois une préparation entièrement résolue — rapatriement des entités compris, et non à `geoleaf:cache:completed`, émis avant lui —, après une purge, et quand la file ou le réseau bougent. Le bloc ne juge rien : le verdict est celui du core. Caché sur un core plus ancien | `cache/preflight-block.ts`                                     |
