...
> les gardes neuves re-vues rouges par mutation, et prouvé sur le bundle livré ; l'instance de
> `createConnector` (CN-26), qui ne touche plus au renouvellement de la page, vue rouge sur le bundle avant son correctif ; et la reprise
> demandée avant le câblage du moteur (CN-38), vue rouge sur le bundle avant son correctif ; et les
> événements de session bornés à leur session (CN-39), vus rouges sur le bundle de même ; et la
> pause après un jeton renouvelé refusé aussitôt (CN-40), la boucle de reprise vue rouge sur le bundle
> avant son correctif. Le **cycle de
...
| CN-03 | HTTPS imposé hors développement local                                       | `baseUrl` en `http://`                                                                                                              | Refusé, sauf quand la PAGE tourne sur un hôte de développement — `localhost`, `*.localhost`, boucle locale (`127.0.0.0/8`, `::1`), `*.test` (RFC 6761) — où un avertissement console suffit ; `.local` exclu, il nomme de vrais intranets   | `config.ts`                                                                                           |
...
| CN-32 | La file écartée par une session morte repart                                | `authenticated` ou `token-refreshed` de SA session (CN-39)                                                                          | `Storage.requeueAll("authRequired")` puis `Storage.pushOutbox()` — deux gestes PUBLICS, aucun import du cœur ; sans capacité hors ligne, rien ne se passe                                                                                   | `session-resume.ts`                                                                                   |
...
| CN-38 | Une reprise demandée avant le câblage du moteur est rejouée                 | `token-refreshed` ou `authenticated` quand `Storage` répond `engineUnavailable` — un `configure()` avant `GeoLeaf.boot()`           | La reprise attend `Storage.whenReady()`, rejoint la passe d'armement du cœur, puis remet la file en route ; une seule attente par page, levée au désarmement ; sans `whenReady`, un seul essai                                              | `session-resume.ts`                                                                                   |
| CN-39 | Un événement de session ne concerne que sa session                          | `token-refreshed` ou `authenticated` qui nomme une autre `baseUrl` — une instance de `createConnector()`, une autre copie du plugin | Ni reprise de la file (CN-32) ni désarmement de la relance (CN-35). Un événement qui ne nomme aucune session compte encore : seul ce qui est connu pour être à une autre est ignoré                                                         | `session-resume.ts` (`concernsSession`), `renewal-retry.ts`                                           |
| CN-40 | Un jeton renouvelé que le serveur refuse aussitôt suspend le renouvellement | 401 sur le rejeu avec le jeton neuf, mode `auth.endpoint`                                                                           | Session gardée, 401 rendu, ni `auth-error` ni fenêtre ; aucun renouvellement pendant une durée d'échange, sauf aux déclencheurs de la relance (CN-35). La reprise de la file (CN-32) ne boucle plus sur un serveur qui renouvelle et refuse | `fetch-interceptor.ts`, `token-store.ts` (`holdRenewals`)                                             |
...
🛑 **CN-38 : une session peut revenir AVANT le moteur hors ligne.** `GeoLeaf.Storage` existe dès
l'import du cœur, mais son moteur n'est câblé qu'au chargement de la capacité hors ligne, pendant
le boot. Un `configure()` appelé avant `GeoLeaf.boot()` — l'ordre de `apps/geoleaf-app/init.js` —
renouvelle une session expirée à cet instant, et les deux gestes répondent `engineUnavailable`. La
passe d'armement du cœur ne rattrape pas ce manque : elle rejoue `pending` et `failed`, jamais une
quarantaine. La reprise attend donc `Storage.whenReady()`, rejoint d'abord la passe que le cœur
lance à cet instant — elle a lu la file avant toute remise en file —, puis rejoue. Prouvé sur le
bundle livré par `e2e/49-connector-resume-before-engine.spec.js`, dont le témoin est
l'avertissement « moteur hors-ligne non câblé ».
...
`fetch`, ni crochet d'ouvrier, ni pont de tuiles, et jamais de fenêtre de connexion, `auth.ui`
compris. Son `destroy()` neutralise ses lectures de jeton ;
...
🛑 **L'instance renouvelle pour elle-même, et ne touche jamais au renouvellement de la page.**
Le magasin de jetons tient un délégué de page, que `configure()` seul pose et retire — à chaque
appel, `null` sans `auth.endpoint`. Une instance créée avec `auth.endpoint` garde sa fonction de
renouvellement pour ses propres lectures (`getTokenAsync`), et son `destroy()` ne retire rien.
Jusqu'au 18/09/2026, elle installait la sienne **à la place** de celle de la page — la session du
singleton se renouvelait alors auprès du point de renouvellement de l'instance — et `destroy()` la
retirait **quel qu'en soit l'auteur** : le 401 suivant du singleton ne trouvait plus rien pour
renouveler, et la session prenait fin sans qu'aucun serveur l'ait refusée. Prouvé sur le bundle
livré par `e2e/51-connector-instance-renewal.spec.js`, rouge avant le correctif.

⚠️ **Une session reste celle d'une `baseUrl`.** Une instance et le singleton sur la même API
partagent le jeton stocké, et un renouvellement en vol, lancé par l'un, est rejoint par l'autre.
...
| --------------------------------------------- | ---------------------------------------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------- |
| `geoleaf:connector:authenticated`             | Fenêtre de connexion                                       | `{ baseUrl }`                | Relance la file (CN-32) et désarme la relance (CN-35) — de SA session seulement (CN-39)       |
| `geoleaf:connector:token-refreshed`           | Renouvellement — celui de la page, ou celui d'une instance | `{ baseUrl }`                | Relance aussi la file (CN-32) et désarme la relance (CN-35) — de SA session seulement (CN-39) |
...
| **Un jeton renouvelé refusé aussitôt suspend le renouvellement, sans clore la session** | Un serveur qui renouvelle puis refuse ce qu'il émet — clé de signature désynchronisée entre deux nœuds, audience fausse — faisait boucler la reprise de la file, sans borne, dès qu'une remise en file lente lui faisait ouvrir une passe. Clore la session serait dur pour un serveur à moitié mal configuré, où certains nœuds acceptent le jeton : elle est gardée, et le renouvellement suspendu une durée d'échange, comme en panne                 | Clore la session au premier jeton renouvelé refusé            |
...
| **HTTPS toléré sur le développement local seulement**                                   | Un développement local sur `http://` doit rester possible ; une mise en production sur du non chiffré transporterait le jeton en clair. La liste ne tolère donc qu'un nom qu'AUCUN hôte de production ne peut porter : ceux que les navigateurs tiennent eux-mêmes pour des contextes sécurisés, et `.test`, réservé aux tests et jamais délégué dans le DNS public (élargie en 1.3.2 ; l'hôte jugé reste celui de la page, pas la destination du jeton) | Imposer HTTPS partout                                         |
