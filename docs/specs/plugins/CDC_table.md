...
| TB-05 | Défilement **virtuel**                     | Couche volumineuse                                   | Seules les lignes visibles sont dans le DOM — et la fenêtre est une **vue du modèle** : sa hauteur et ses intercalaires suivent l'ensemble que la recherche laisse visible, pas l'effectif de la couche. Le nombre de lignes retenues est borné — voir §Configuration                                                                                                                                                                                                                                                            | `table-renderer-virtual-scroll.ts`, `view-model.ts`           |
| TB-06 | Sélection bidirectionnelle                 | Clic sur une ligne, ou appel programmatique          | La sélection est un état partagé : la table la reflète, la carte la surligne. **Tout se décide sur le MODÈLE, jamais sur les lignes rendues** — « tout sélectionner » prend l'ensemble visible entier (sous recherche : son résultat), la sélection par plage part d'une **ancre**, le dernier clic unitaire, et la case d'en-tête compare la sélection à l'effectif visible                                                                                                                                                     | `selection-actions.ts`, `table-selection.ts`, `view-model.ts` |
...
| TB-11 | Résolveur d'identifiant **unique**         | Nom de repère, attribut DOM, table de correspondance | Les trois passent par le même résolveur — sinon la sélection et l'export désigneraient des entités différentes. ⚠️ L'identité est fixée **une fois, au chargement**, par `view-model.rebuild` ; le rendu la reçoit et n'en frappe aucune. Un tri réordonne les lignes, il ne les renumérote pas                                                                                                                                                                                                                                  | `feature-id.ts`, `view-model.ts`                              |
...
| TB-16 | Recherche plein texte **sur le modèle**    | Saisie dans le champ de la barre d'outils            | Filtre débouncé (300 ms) sur les **valeurs du modèle**, formatées comme la cellule les affiche — donc ce que l'utilisateur lit est ce que la recherche compare. Il restreint l'ensemble visible, que le rendu projette : il survit au défilement, et un compteur « n / total » dit que la vue est un sous-ensemble. Quand la couche déclare `searchFields`, seules ces propriétés sont interrogées                                                                                                                               | `panel.ts` → `createSearchInput`, `view-model.ts`             |
...
🛑 **`virtualScrolling` était dans le même cas, et il est clos de la même façon depuis la tâche
2.7 (ANO-039).** La bascule en rendu virtuel se décide sur le seuil `VIRTUAL_THRESHOLD`
(`table-renderer-virtual-scroll.ts`), lu par `renderer.ts` — **jamais sur la clé de profil**.
L'éteindre n'éteint rien.

Il a donc quitté `DEFAULTS` **et les profils livrés** — `profiles/_reference` le déclarait à
`false` pendant que la virtualisation tournait, ce qui était un profil qui ment — mais il **reste
dans le type publié, marqué `@deprecated`**, pour le motif déjà écrit sur son jumeau : supprimer un
membre d'un type publié casse la compilation d'un intégrateur qui l'a écrit, tandis que le
déprécier le lui dit. Il n'a pas sa place dans `DEPRECATIONS.json` — une clé sans effet n'a aucun
remplaçant, donc ce n'est pas une dépréciation au sens de la politique ; il est exempté **nommément**
dans `CC-10` (`scripts/verify-consumer-contract.cjs`), avec son motif.

```bash
grep -rn "virtualScrolling" packages/plugins/table/src/ | grep -v __tests__
```

⚠️ **Fermé par la PAROLE, pas par le câblage**, et c'est délibéré : rendre la clé effective
imposerait, pour la tenir, un second plafond contre le cas `false` sur 30 000 lignes — c'est-à-dire
réintroduire ce que le seuil règle déjà.
...
| `searchFields` | `array`   | Propriétés interrogées par la recherche. Absent ou vide → toutes les colonnes. Les deux formes publiées sont acceptées |
