...
- **Il ne reprojette pas.** L'emprise est extraite, les pixels ne sont pas reprojetés : un fichier
  ...
  être servi en **EPSG:4326**. ⚠️ Cette ligne a écrit « EPSG:4326 ou 3857 », et 3857 ne place pas :
  `injectImageSource` passe l'emprise rendue par `getBoundingBox()` **telle quelle** dans les
  `coordinates` de la source image MapLibre, qui sont des degrés — sur un fichier en 3857 ce sont
  des mètres, et l'image se pose hors du monde. Aucune conversion n'existe dans le plugin. Le même
  énoncé vit dans `packages/plugins/cog/README.md` §Limitations : les deux se corrigent ensemble.
...
L'auto-enregistrement d'`entry.ts` est **immédiat** : il appelle `plugins.register`, pas
`registerLazy`. ⚠️ **Cela ne dit rien de la façon dont le bundle ARRIVE, et confondre les deux est
le piège.** Depuis le 07/08/2026 l'application déployable charge `cog` **paresseusement** —
`gl.plugins.registerLazy("cog", () => import("./dist/geoleaf-cog.plugin.js"))` dans
`apps/geoleaf-app/init.js` —, et sa balise `<script>` a quitté `index.html` pour cette raison : il
était le plus lourd de la flotte, sur le chemin critique de chaque chargement, pour une API que la
plupart des pages n'appellent jamais. L'appel vit entre une paire de marqueurs
`GEOLEAF-DEPLOY:GATED-BLOCK cog` que `build-deploy.cjs` retire des variantes qui n'embarquent pas
le bundle — sans ce retrait `isLazyAvailable("cog")` rendrait vrai et le premier
`plugins.load("cog")` partirait sur un 404 (gardé par APP-07). **Le plugin ne s'enregistre pas
paresseusement ; c'est l'hôte qui le charge paresseusement**, et rien ne le déclenche de lui-même —
ni écouteur, ni créneau de barre d'outils, ni couche déclarative : le consommateur appelle
`plugins.load("cog")`.
...
| CG-13 | Canevas hors écran quand UTILISABLE          | `OffscreenCanvas` rendant un contexte 2D                             | Décodage sur `OffscreenCanvas` ; **repli** sur un canevas de document sinon — ⚠️ la condition est l'obtention effective du contexte, pas la seule présence du constructeur                                                                                                                                                                                                                                                                                               | `cog-renderer.ts`                                                          |
...
| CG-16 | Mise à jour à chaud                          | `handle.update({ opacity: 0.5 })`                                    | La couche est retirée puis re-rendue à partir du `CogInfo` **mémorisé** : `getCogInfo` n'est pas rappelé. ⚠️ Les en-têtes TIFF, eux, **sont bien relus** — `executeCogPipeline` passe par `loadViewportRasters`, qui rouvre le fichier par `openCog` → `fromUrl`. Ce qui est épargné est un appel de métadonnées, pas un aller-retour réseau                                                                                                                             | `cog-api.ts` → `addLayer`                                                  |
| CG-17 | Retrait                                      | `handle.remove()` ou `removeLayer(map, id)`                          | Couche **et** source retirées                                                                                                                                                                                                                                                                                                                                                                                                                                            | `cog-renderer.ts` → `removeImageSource`                                    |
| CG-18 | Annulation                                   | `signal` d'un contrôleur d'abandon                                   | La promesse rejette aussitôt sur un `AbortError` — ⚠️ **la requête en vol n'est PAS interrompue** : `signal` n'est jamais transmis à `fromUrl`, l'abandon est une simple **course de promesses**. Et seule l'ouverture est courue : `readRasters` ne l'est pas, donc un abandon pendant le décodage ne fait rien                                                                                                                                                         | `cog-loader.ts` → `openCog`                                                |
| CG-19 | **La couche traverse un changement de fond** | Basculement vers ou depuis un fond vectoriel — le style est remplacé | Source image et couche raster portées dans le style entrant, à leur place : `injectImageSource` les déclare à l'adaptateur qui pilote la carte REÇUE (`declareOwnedStyleIds`, core ≥ 3.10.0 — cherché parmi les cartes de l'hôte), `removeImageSource` retire la déclaration. ⚠️ Posées hors du registre de l'adaptateur, elles étaient effacées par le diff. Une carte que l'intégrateur pilote seul, ou un cœur plus ancien : rien n'est déclaré, comportement d'avant |

Les tests qui couvrent ces lignes : `packages/plugins/cog/src/__tests__/` — emplacement imposé par
le contrat (PC-09) — **et `e2e/17-cog.spec.js`**, que cette ligne omettait ; CG-19 a aussi sa preuve
navigateur, `e2e/58-style-swap-keeps-foreign-layers.spec.js`. Le spec est le seul
endroit qui éprouve le plugin de bout en bout : il vise `deploy-full`, appelle explicitement
`plugins.load("cog")`, et sert un vrai GeoTIFF (`e2e/fixtures/sample-cog.tif`, engendré dans le
dépôt par `e2e/fixtures/_gen-cog.cjs`) par des requêtes HTTP **à plage**, pour que le `geotiff.js`
réellement empaqueté décode de vrais octets. C'est lui qui verrouille `getInfo().epsg`, donc le
chemin `getGeoKeys()` de CG-03 — celui qu'aucune doublure de test ne prend.
...
⚠️ **Conséquence pratique** : rien ne peut « désactiver » ce plugin **par profil**. Il n'est pas
pour autant toujours livré — et c'est un contrat que cette fiche ignorait. Son `package.json` porte
sous la clé `geoleaf` le **descripteur de livraison** que lit `scripts/lib/discover-plugins.cjs` :
`bundle`, `lazyChunks` et `includeFlag`, **tous trois obligatoires**. Un descripteur absent ou
incomplet **arrête** le build au lieu de prendre un défaut, et le motif est écrit sur place : un
défaut ne distingue pas « décidé » d'« oublié », or ce manifeste décide de ce qui part chez un
client. `cog` est l'un des rares plugins dont l'`includeFlag` n'est pas nul — c'est `includeCog` :
`deploy-full` l'embarque, `deploy-core` non, et `build-deploy.cjs` retire alors sa balise et son
bloc gaté d'`init.js`. La matrice des variantes vit dans `scripts/build-deploy.cjs` ; elle ne se
recopie pas ici.
...
| **Canevas hors écran quand il est UTILISABLE, pas quand il existe**          | Décoder un gros raster sur le canevas du document bloque le fil principal ; le repli reste correct, simplement moins fluide. ⚠️ La condition porte sur l'**obtention effective** du contexte 2D : un environnement peut exposer le constructeur et rendre `null` — la sonde ne coûte rien, deux `getContext("2d")` sur une même surface rendent le même contexte | Exiger le canevas hors écran — exclurait des navigateurs encore en service · se fier au seul `typeof OffscreenCanvas` — c'est la forme qui a rendu une surface sans contexte, et `rastersToCanvas` jetait une ligne plus loin |
...
| **Enregistrement immédiat dans `entry.ts`, chargement PARESSEUX par l'hôte** | Le plugin n'a ni action de barre d'outils ni écouteur à attendre : une fois son bundle évalué, son API doit être là. Mais le bundle, lui, n'a pas à être sur le chemin critique — c'était le plus lourd de la flotte —, d'où le `registerLazy` d'`init.js` depuis le 07/08/2026, déclenché par un `plugins.load("cog")` explicite                                | Une balise `<script>` eager dans `index.html` : c'était la forme d'origine, et elle a été retirée pour ce motif                                                                                                               |
...
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 13 contrôles        | **Il n'y a pas de PC-06** — les contrôles sont PC-01…PC-05 et PC-07…**PC-14**, plus PC-04-WIDE. ⚠️ Cette case s'arrêtait à PC-13 : `PC-14` (INV-CONFIG — la `profileKey` d'un plugin sous `modules.<pluginId>`) a été posé le 17/08/2026, et l'énumération se relit dans l'en-tête de `scripts/verify-plugin-contract.cjs`, jamais ici                                                                                                                                            |
| 13 conformes        | **Autant de plugins que le dépôt en publie** — `node scripts/verify-plugin-contract.cjs` rend le ratio. ⚠️ Cette case a recopié « **13** plugins », puis « **12** » après la fusion d'`addpoi` dans `editor` — justes à leur date, faux depuis : la flotte a grandi (`position-share`, puis `routing` et `navigation`). Un compte recopié dans la case même qui pose qu'il se **mesure** se périme comme les autres ; il se lit à la commande qui précède, et nulle part ailleurs |
...
(emplacement des tests). PC-07, PC-13 et **PC-14** ne le concernent pas : **il n'écrit ni HTML ni CSS**, et il ne déclare aucune `profileKey` — `grep -rn "profileKey" packages/plugins/cog/src/` ne rend rien, ce qui est la conséquence directe du §Configuration ci-dessus.
...
| `@geoleaf/host-runtime`      | Déclarée en **dépendance de développement**, regroupée à la construction : un type dans `entry.ts`, et le seam `declareOwnedStyleIds` dans `cog-renderer.ts` (CG-19) |

⚠️ **`cog` consomme `host-runtime` à deux niveaux, et le premier reste un écart de génération.**
`entry.ts` n'en prend qu'un type (`GeoLeafHost`) et écrit `globalThis as { GeoLeaf?: … }` à la main,
là où les plugins plus récents importent la fonction d'accès (`getGeoLeaf()`) — ce n'est pas une
non-conformité, aucun contrôle du contrat ne l'exige, et la forme récente est celle de `geocoding`.
Depuis la 1.0.4, `cog-renderer.ts` en importe aussi une **valeur** : `declareOwnedStyleIds`, le seam
par lequel sa couche traverse un changement de fond (CG-19). ⚠️ Cette fiche a longtemps dit « **en
type seulement** » : c'était vrai jusque-là, et ne l'est plus.
...
- **Au-delà du montage, un seul accès au namespace : la déclaration de CG-19.** Le plugin écrit
...
| ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| « le plugin est **lazy-loadé** »                                                                                               | **Vrai de l'hôte, faux du plugin** : `entry.ts` appelle bien `plugins.register`, mais `init.js` le déclare par `registerLazy` depuis le 07/08/2026. Cette case a rendu « Faux » sans réserve — le CDC avait raison sur le chargement, tort sur l'enregistrement |
