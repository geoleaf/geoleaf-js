...
| CG-13 | Canevas hors écran quand UTILISABLE    | `OffscreenCanvas` rendant un contexte 2D       | Décodage sur `OffscreenCanvas` ; **repli** sur un canevas de document sinon — ⚠️ la condition est l'obtention effective du contexte, pas la seule présence du constructeur (B-258) | `cog-renderer.ts`                                                          |
