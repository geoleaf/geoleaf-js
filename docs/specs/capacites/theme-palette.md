...
| TP-09 | Bouton dans le bandeau d'onglets desktop        | Seam `geoleaf:desktop-panel:tabs-ready`, capacité activée, ≥ 2 palettes | `.gl-rp-tab-btn.gl-rp-palette-btn` inséré **avant** `.gl-rp-theme-toggle` (repli : appendu en fin de bandeau) | `palette-button.ts` → `appendPaletteButtonToTabs` |
