...
- **Elle n'affiche pas le badge de connectivité** — elle en déclare seulement le drapeau, que
  `offline` lit.
- **Elle ne touche pas aux service workers qu'elle n'a pas enregistrés.** Ni au boot, ni au
  démontage : seul `sw-core.js` la regarde. La règle existe parce que ce bundle est monté dans des
  applications hôtes qui ont le leur.
...
| ----- | ----------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| PW-01 | Enregistrement du service worker                | `modules.pwa.enabled === true`                | Service worker unifié enregistré avec la portée `"./"` — ce qui **supporte les déploiements en sous-chemin** — mais **hors du chemin critique** : l'appel part dans `Helpers.lazyExecute` (`requestIdleCallback` + plafond `SW_REGISTER_IDLE_CEILING_MS`), donc il n'a **pas** eu lieu au retour de `init`                                              | `lifecycle.ts` → `init`                          |
| PW-02 | Désenregistrement quand le gate est fermé       | `enabled` absent ou différent de `true`       | Le service worker **de GeoLeaf** (`sw-core.js`) est désenregistré — un visiteur qui revient obtient bien « pas de SW ». 🛑 **Les autres service workers de l'origine sont laissés intacts** : le bundle est monté dans des applications hôtes tierces, et les désenregistrer tous a retiré à l'hôte son mode hors ligne et son installation, en silence | `lifecycle.ts` → `_unregisterOwn`                |
...
| PW-14 | Démontage complet                               | `sharedTeardown()` / `_reset()`               | Écouteurs globaux de l'invite Android relâchés, minuteur iOS annulé, bannières retirées, service worker **de GeoLeaf** désenregistré — jamais ceux de l'hôte                                                                                                                                                                                            | `lifecycle.ts` → `_reset`, `pwa-manager.ts`      |
...
| PW-16 | Déclaration introspectable                      | `getCapabilitySchema("pwa")`                  | Toutes les clés de premier niveau de la table §Configuration — mais **cinq seulement** annotées de qui les lit : `name` et `short_name` en `RUNTIME + BUILD`, `description`, `theme_color` et `background_color` en `BUILD-TIME ONLY`. `enabled`, `installPrompt` et `offlineDetector` ne disent pas leur lecteur                                       | `pwa-capability.ts`                              |
...
**Aucun événement GeoLeaf**, dans les deux sens. La capacité écoute en revanche deux signaux
**globaux** du navigateur, tous deux sur le chemin Android : `beforeinstallprompt`, qu'elle diffère
pour proposer sa propre bannière, et `appinstalled`, qui retire la bannière quand l'application a
été installée par un autre chemin. S'y ajoutent les `click` de son propre DOM de bannière, qui ne
survivent pas au retrait de celle-ci.

⚠️ Les deux écouteurs globaux sont posés avec des handlers **nommés**, et ce n'est pas un détail de
style : c'est la condition pour que `_reset()` détache exactement les mêmes références — une
fermeture en ligne fuirait à chaque `initApp()` (`install-prompt.ts` → `init` / `_reset`).
...
| **…mais SEULEMENT le service worker de GeoLeaf**                       | 🛑 Défaut bloquant mesuré chez un hôte : monté dans une application tierce avec le gate fermé, le balayage de `getRegistrations()` désenregistrait le worker de l'**hôte**, qui perdait son mode hors ligne et son installation sans aucun message. La portée d'une désinscription est celle de ce qu'on a enregistré soi-même. Le prédicat porte sur le dernier segment de `scriptURL`, jamais sur un suffixe — un `vendor-sw-core.js` de l'hôte n'est pas le nôtre                                                                                                                                                                                                                                                                                                            | Balayer l'origine ; ou ne rien désenregistrer du tout                  |
...
| **Enregistrement du SW différé jusqu'à l'inactivité, avec un plafond** | L'installation récupère tout `STATIC_ASSETS`, et elle démarrait à l'instant où ce module s'initialisait : en concurrence directe avec les premières tuiles et les couches du profil, sur la même connexion. 🛑 `geoleaf:app:ready` était le crochet évident et il a été **écarté sur mesure** — cet événement n'est émis que par `UIModule`, trié après `geojson`, donc s'y accrocher plaçait l'enregistrement derrière l'attente réseau **non bornée** de `GeoJSONModule.init()` et le supprimait quand `beforeBoot` rejette ou que `registry.init()` jette : précisément les boots où un cache hors-ligne vaut le plus. Le `timeout` de `requestIdleCallback` garde les deux propriétés — céder la place à la carte, et s'exécuter quand même si l'inactivité ne vient jamais | S'accrocher à `geoleaf:app:ready` · enregistrer sur le chemin critique |
...
n'entre donc pas dans le tri topologique du registre : la question de son rang de dépendance **ne
se pose pas ici**.
...
(`applyCssText`) et `utils/general/helpers-namespace` (`Helpers.lazyExecute`, qui sort
l'enregistrement du service worker du chemin critique). **Aucun accès à la carte**, aucun `IMapAdapter`. **Aucune référence à un plugin**
...
```bash
grep -o '"\(name\|short_name\|description\|theme_color\|background_color\)":[^,}]*' deploy/deploy-full/manifest.json
```
...
✅ **Et sa DÉCLARATION ne porte plus aucune citation de ligne.** Cette section en annonçait cinq,
périmées et **publiées** — `getCapabilitySchema('pwa')` sert les descriptions du schéma aux
intégrateurs et à un studio de configuration, un lecteur qui est **hors du dépôt** et ne peut donc
pas constater la dérive. Elles ont été résolues en **symboles** (`PWAManager.init`,
`PwaLifecycle.init`, « manifest merge ») : un symbole survit au déplacement et reste vérifiable au
grep, une ligne non.

⚠️ **Et la classe est fermée, pas seulement le lot** : `__tests__/guards/no-line-citations-in-published.guard.test.js`
interdit la FORME `fichier.ext:ligne` sur les deux seules surfaces publiées — les `description` des
`configSchema` et les façades `src/api/geoleaf.*.ts`. Il ne juge pas la véracité d'une phrase, qui
reste à un humain. Vérification sur cette capacité :

```bash
grep -nE '\.(ts|js|cjs|mjs):[0-9]+' packages/core/src/capabilities/pwa/*.ts
```
