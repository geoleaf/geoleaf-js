...
> déployé** (`scripts/build-deploy.cjs`, qui fusionne le bloc dans le `manifest.json` livré), par
...
la construction écrit dans le `manifest.json` déployé.
...
- **Elle ne génère pas le `manifest.json`.** Elle en **déclare** les champs ; c'est
...
| PW-15 | Champs du manifeste fusionnés à la construction | `npm run build:deploy`                        | `name`, `short_name`, `description`, `theme_color`, `background_color` écrits dans le `manifest.json` déployé                                                    | `scripts/build-deploy.cjs`                       |
...
`manifest-only-keys.test.js`, qui **épingle** le partage entre les clés lues à l'exécution et
...
| **`persist()` ici, et pas dans le service worker**            | `navigator.storage` est une API **`Window`** : elle est **absente** de la portée du worker, donc `sw-core.js` ne peut pas la porter. Ce cycle de vie est le premier code `Window` qui tourne derrière le gate PWA                                                                                                                                                                                        | L'appeler depuis `sw-core.js`                         |
...
Le fichier incriminé se contredisait d'ailleurs déjà lui-même : `manifest.full.ts` écrit
...
`profiles/geoleaf.config.json` et fusionne cinq champs sur le gabarit `manifest.json` de
