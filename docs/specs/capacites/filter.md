...
| FI-09 | Recherche textuelle tolérante                           | `text`                                         | Tous les mots présents, **dans n'importe quel ordre**, insensible à la casse **et aux accents** — la règle vit dans `utils/general/normalize-text.ts`, partagée avec `GeoLeaf.Layers.search` : les mêmes mots trouvent les mêmes entités qu'on filtre la carte ou qu'on la cherche | `engine/predicate.ts` → `utils/general/normalize-text.ts` |
