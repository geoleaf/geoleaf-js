...
| ----- | ------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| TS-01 | Montage différé au dévoilement                    | `geoleaf:app:ready`                                                  | La barre se construit à ce moment-là, **jamais** avant. Abonnement par `whenAppReady()` : écoute `{ once: true }` avant le dévoilement, montage immédiat si l'`init()` arrive après | `lifecycle.ts` → `init`, `_onAppReady`                     |
...
| ---------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `geoleaf:app:ready`    | **écouté**, par `whenAppReady()` | Le déclencheur de montage — dispatché par le dévoilement de l'application. `{ once }` avant le dévoilement, montage immédiat après |
...
| **Montage sur `app:ready`, pas à l'`init()` du module**            | La barre a besoin d'un profil actif et de deux conteneurs que le kernel construit plus tard. L'abonnement passe par `whenAppReady()`, qui capte l'événement encore à venir et monte aussitôt s'il est passé. 🛑 Cette case concluait « l'événement est asynchrone, donc il est toujours capté » : faux sur un profil **sans thème par défaut**, où il partait de l'`init()` de `ui`, dépilé avant ce module (signalé sur 3.10.3)                                       | Monter dans `init()`                                                |
...
**Ce que `init()` fait réellement** : il s'abonne à `app:ready` par `whenAppReady()` — **un**
écouteur `{ once: true }`, ou le montage immédiat si l'application est déjà dévoilée.
...
🛑 **Ce raisonnement ne regardait qu'un émetteur, `theme-engine`, et il y en avait deux.** Sur un
profil **sans thème par défaut**, c'était `ui` qui dévoilait l'application depuis son `init()` — et
`ui` est dépilé **avant** cette capacité : l'écouteur arrivait après le tir, et la barre ne montait
jamais (signalé sur 3.10.3, avec la légende, l'échelle, les coordonnées et le filtre). Le
dévoilement « sans thème » est désormais libéré par le boot à la fin de `registry.init()`, et
`whenAppReady()` monte aussitôt un abonné tardif. **L'ordre entre écouteurs, lui, ne bouge pas** :
avant le dévoilement, ils sont posés dans l'ordre des `init()`, comme avant.
...
| `kernel/shared/index.js` (`whenAppReady`)  | **Baril de médiation** — l'abonnement du montage (26/09/2026) |
