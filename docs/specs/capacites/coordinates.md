...
| ----- | ------------------------------------------------ | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| CO-01 | Montage différé                                  | Événement `geoleaf:app:ready`           | Le relevé apparaît une fois la carte **et** l'UI différée en place — abonnement par `whenAppReady()` : écouteur `{ once: true }` avant le dévoilement, montage immédiat après | `lifecycle.ts` → `init`, `_onAppReady`                          |
...
| Événement           | Sens                             | Rôle                                                                                                                                                        |
| ------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `geoleaf:app:ready` | **écouté**, par `whenAppReady()` | Déclenche le montage — préserve le moment historique du montage différé. `{ once: true }` avant le dévoilement, montage immédiat si l'`init()` arrive après |
...
| **Montage sur `geoleaf:app:ready`, pas à l'`init()` du module** | Le relevé a besoin de l'UI différée — dont la barre d'échelle. L'abonnement passe par `whenAppReady()` : il capte l'événement encore à venir, et monte aussitôt s'il est passé. 🛑 Cette case disait « l'événement est asynchrone par rapport à l'initialisation des modules, donc s'abonner pendant l'`init()` suffit à le capter » — faux sur un profil **sans thème par défaut**, où il partait de l'`init()` de `ui`, dépilé avant ce module, et le relevé ne montait jamais (signalé sur 3.10.3) | Monter directement — l'amarrage échouerait presque toujours et le repli s'appliquerait par défaut                    |
| **Dépendance sur `geojson`, pas sur `ui`**                      | Le rang place le module dans la vague tardive, avant `theme-engine`, qui émet l'événement sur le chemin thémé ; c'est le même choix que `labels` et `theme-selector`. Ce rang ne suffisait pas sur un profil sans thème par défaut (ligne précédente) ; `whenAppReady()` ne dépend plus de lui                                                                                                                                                                                                        | Dépendre de l'UI — l'abonnement arriverait après le tir                                                              |
...
Deux accès kernel : `kernel/config/config-primitives.js`, **exception nommée par la règle**, et le
baril de médiation `kernel/shared/index.js` (`whenAppReady`, depuis le 26/09/2026). Le
