...
⚠️ **Mais la couche basculée perd le filtre du panneau** (VT-15). Le filtre juge les entités en
mémoire, et une couche en tuiles n'en garde aucune : sous n'importe quel champ, elle reste entière.
Une couche que les utilisateurs filtrent ne se bascule donc pas sans renoncer à ce filtre. La
recherche locale (`GeoLeaf.Layers.search`) ne la lit pas non plus : même cause, et c'est écrit dans
[`CDC_kernel.md`](../CDC_kernel.md). Les étiquettes, elles, suivent la couche (VT-14).
...
| VT-14 | Étiquettes de la couche                       | Style portant un objet `label`, ou `labels.enabled` dans la définition | Les étiquettes sont armées au chargement, comme sur le chemin GeoJSON, et la couche de symboles lit la _source-layer_ que l'adaptateur a enregistrée pour la couche. Jusqu'au 26/09/2026, elle n'en nommait aucune : MapLibre la refusait (un événement `error`, jamais une exception) et le bouton restait allumé sans rien dessiner. Une couche créée par `Layers.create()` n'armait pas non plus ses étiquettes | `vector-tiles.ts` → `loadVectorTileLayer`, `capabilities/labels/label-renderer.ts` |
| VT-15 | **Aucun filtre du panneau**                   | Filtre appliqué, `Layers.setVisibleSubset`                             | La couche reste entière : le prédicat du filtre s'exécute sur les entités en mémoire, et l'entrée d'une couche MVT n'en porte aucune (`features: []`)                                                                                                                                                                                                                                                              | `kernel/geojson/geojson-filter.ts` → `_applyFeatureVisibilityForLayer`             |
...
(`types.ts` → `LayerConfigModuleLike`, `LayerManagerModuleLike`). Elle lit aussi `GeoLeaf.Labels`,
pour armer les étiquettes d'une couche chargée (VT-14) : ce membre-là est typé sur le namespace
lui-même, sans type structurel local. Deux de ces membres reposent
