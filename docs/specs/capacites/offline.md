...
| OF-14 | Base locale en modules enregistrés              | —                                         | Images, couches, préférences — et depuis la **v4** (tâche 3.4) `features` (une entité par enregistrement) et `outbox` (la file d'écritures qui a remplacé `sync_queue`, retiré à la tâche 4.11) : **sept modules**, un registre **interne** (plus de global)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `db/db-modules-registry.ts`                                                                                                                                |
...
| `discardQuarantined(id, localId)`  | **Destruction confirmée**, qui rend l'entité à la vérité du serveur (25/09/2026) — voir ci-dessous             |
...
#### `discardQuarantined()` — la saisie détruite rend l'entité à la vérité du serveur

La destruction ne part que sur confirmation : l'appelant rend le `localId` de l'entrée, qu'il ne
connaît qu'en l'ayant listée. Puis l'enregistrement local de l'entité revient à la vérité du
serveur, **sauf si une autre entrée de la file la nomme encore**, auquel cas c'est du travail vivant
et rien n'y touche :

- **sans identité serveur** (une création jamais arrivée) **ou `deletedOnServer`** : l'enregistrement
  est retiré, puisque le serveur n'a rien à rendre ;
- **sinon** (modification ou suppression refusée, budget épuisé…) : il repasse `synced`. Le prochain
  rapatriement le remplace par la version du serveur, et un rapatriement complet qui ne la sert plus
  le balaie. Jusque-là, le contenu local reste affiché.

🛑 **Avant le 25/09/2026, la destruction ne retirait que l'entrée de file.** La quarantaine s'écrit
sur l'entrée, pas sur l'enregistrement, qui gardait son `pending`. Il restait donc un enregistrement
« en attente » que rien ne pousserait, et que le rapatriement préserve puisqu'il n'est pas `synced`.
Une couche qui lit l'appareil avant le réseau le dessinait à chaque chargement, en ligne aussi.
L'opérateur voyait rester à l'écran ce qu'il venait de détruire. Mesuré par lecture du code ; la
preuve est dans `__tests__/capabilities/offline/quarantine-api.test.ts`.
