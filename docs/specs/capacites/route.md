...
- **Elle n'agrège pas un itinéraire multi-segments** : les extrémités sont dérivées **par entité**.
  ⚠️ Cette ligne disait aussi « et ne pose pas d'étapes intermédiaires » — **faux depuis le
  21/08/2026** (RT-10/RT-11). Elle n'en _dérive_ toujours aucune, et c'est le point qui compte :
  rien, dans une suite de sommets, ne dit lesquels sont des étapes, donc les dériver reviendrait à
  en inventer. Elle sait en revanche **styler** celles que la donnée porte déjà.
- **Elle n'étiquette pas les étapes.** Un numéro d'étape lisible sur la carte demande une
  sous-couche de texte, que le contrat de l'adaptateur n'expose pas aujourd'hui. Les points portent
  `properties.step` : l'étiquette est une ligne de style le jour où cette couture existe.
