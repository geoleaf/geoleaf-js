...
| TR-17 | Toast « profil chargé », **réessayé** — ⚠️ **jamais rendu au boot** | `geoleaf:profile:loaded`                             | Rendu si le renderer est prêt ; sinon **mémorisé** et retenté sur `geoleaf:map:ready`. ⚠️ **L'écouteur arrive après l'événement** : `init()` le pose pendant `registry.init()`, et `geoleaf:profile:loaded` part avant, pendant `loadActiveProfileResources()` — une seule fois par page, un changement de profil rechargeant la page. La bibliothèque ne rend donc jamais ce toast | `lifecycle.ts` → `tryShowProfileToast`, `onMapReady`                 |
