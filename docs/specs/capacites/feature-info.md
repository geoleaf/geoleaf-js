...
**Vérifié contre :** `6238592c` (02/08/2026, Sprint 2 de `roadmap_collecte-terrain-offline`)
...
- **Elle n'émet qu'UN signal**, et seulement sur un geste : `geoleaf:popup:action`, au clic d'un
  bouton d'action déclaré. Rien d'autre.
...
| FE-19 | Visionneuse d'images                                 | `type: "gallery"`                        | Ouverture au clic, sur la surface panneau                                                                                                                                                                                                                                           | `render/lightbox.ts`                                    |
...
couche et de `check-config-coverage.cjs`, pas d'ici.
...
`travail/roadmaps/roadmap_collecte-terrain-offline.md`.
...
| `getConfig(layerId)`             | La déclaration de la couche, ou `null`. **Non gaté** — c'est une lecture |

⚠️ **Le nom de fichier de la façade est `geoleaf.featureinfo.ts`, sans tiret**, alors que
...
⚠️ **La question de rang de B-57 ne se pose PAS ici** : la
...
| --------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------- |
| Lecture — infobulle, bulle, panneau     | **core `feature-info`** | ses quatre fichiers de rendu portent « zero dependency on … the field-renderer package » |
...
cette bibliothèque ferait payer au core **30 Ko gz** pour la moitié qu'il sait déjà faire, parce que
son contrat de composant est **bi-mode**. Une version antérieure de la décision voulait précisément
...
six clés déclarées-jamais-lues. **Le Sprint 2 les a soldés le 02/08/2026.** Elle garde ce qu'ils
...
**trois fois** dans les types du core. Le Sprint 2 solde la triplication **et** fait lire la clé —
...
| §3.3 — « Action buttons → dispatch `geoleaf:popup:action` »            | ✅ **SOLDÉ le 29/07/2026 (B-69)** — le bouton est produit et le signal émis (`render/popup-content.ts:283`). ⚠️ Cette ligne disait encore « écartés, zéro émission » le 02/08 : **elle contredisait le §Contrat exposé du même fichier**, qui annonce l'émission depuis juillet. Corrigée                                                                                                                                               |
| §4.1 — « tableau complet des **14** types »                            | ✅ **SOLDÉ au Sprint 2 (02/08/2026)** — le contrat en déclare **24** et le moteur en rend 24, confrontés par une gate de parité. ⚠️ Cette ligne a porté **trois** comptes faux avant d'être résolue : « 16 rendent, `action` écarté » (périmé depuis B-69), « 14 rendent » (oubliait `hero`), puis « 13 rendent / 4 muets ». Un compteur qu'aucune machine ne confronte se trompe à chaque relecture — c'est l'argument même de la gate |
| §1.1bis — CSS « via `@import` dans `geoleaf-main.css` »                | Les cinq feuilles sont importées **par l'installeur**, ce qui les rend élaguables avec la capacité                                                                                                                                                                                                                                                                                                                                      |
...
| §1.1bis — migration cassante des clés héritées de couche               | ✅ **Vérifié exact** — plus aucune clé racine, tout sous `capabilities.feature-info`                                                                                                                                                                                                                                                                                                                                                    |
...
(`globals.core.ts` n'expose que `{ registerDict, getLabel }`) ». **La parenthèse est au présent et
elle est fausse** : `globals.core.ts` monte aujourd'hui quatre membres, `t` compris. Mesuré aussi :
