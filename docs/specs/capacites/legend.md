...
> et les exemples pas à pas sont dans [`packages/core/docs/legend/GeoLeaf_Legend_README.md`](../../../packages/core/docs/legend/GeoLeaf_Legend_README.md). **Les deux ne se recopient pas** — une
...
| ------ | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| LG-01  | Montage différé au démarrage complet                     | `geoleaf:app:ready`                                                                                                         | Le contrôle est construit et ajouté à la carte. Abonnement par `whenAppReady()` : écouteur `{ once: true }` avant le dévoilement, montage **immédiat** si l'`init()` arrive après — un seul montage par cycle de vie                                                                                                                                                                                                                                                                                                                                                              | `lifecycle.ts` → `init`                                                              |
...
| LG-10  | Correspondance de catégorie insensible à la casse        | Table de taxonomie                                                                                                          | Le rapprochement passe par le résolveur de `taxonomy` — la légende portait sa propre correspondance, qui pouvait diverger                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `legend-generator.ts`, `packages/core/src/capabilities/taxonomy/resolver.ts`         |
...
| LG-11b | Résolution d'icône par **deux canaux complémentaires**   | `styleRules[].when.field` désignant une catégorie                                                                           | **12 graphies** résolvent, et c'est leur UNION qui les couvre : canal 1 (`_FIELD_CATEGORY_MAP`) prend les formes `properties.*` et nues, canal 2 (`_ATTRIBUTE_FIELD_KIND`) la famille `attributes.*` ; le canal 2 ne s'exécute que si le canal 1 a échoué. ⚠️ Aucun canal n'est « le bon » — les éprouver séparément avait laissé `attributes.category` et `attributes.subCategory` déclarées par **aucun des deux**                                                                                                                                                              | `legend-generator.ts` → `_applyPointIcon`                                            |
...
| LG-16  | Démontage complet                                        | `Core.destroy()` de la dernière carte (seam de cycle de vie, inscrit par `LegendLifecycle.init`) · `LegendModule.destroy()` | 🛑 **Le seam est le chemin de l'intégrateur** : `LegendModule.destroy()` n'est atteint que par `ModuleRegistry.destroy()`, qu'aucun chemin de production n'appelle — sans l'inscription, un `Core.destroy()` tombé dans la reconstruction différée montait le contrôle sur la carte détruite (« map is not ready », non capturée ; mesuré le 24/09/2026). **Quatre** objets sont annulés — les trois échéances en attente **et le contrôleur des requêtes de style en vol** —, puis le contrôle est retiré, le registre vidé, les références carte / profil / taxonomie relâchées | `legend.ts` → `_reset`, `legend-overlay.ts`                                          |
...
— avec les 22 autres du même gisement, en citant le **symbole** plutôt que la ligne.
...
gouverne : `Legend.init` est montée sur `geoleaf:app:ready`, or cet événement est **causé** par la
fin de l'application du thème — et c'est cette même application qui appelle le seam, une fois par
couche. L'init est donc **structurellement garantie d'arriver après** les appels du kernel ; aucun
réordonnancement ne pouvait les faire coïncider. Une garde présentielle était vraie dès
`registerGlobals`, donc vraie exactement quand il fallait qu'elle soit fausse.

⚠️ **Ce que la garde ne fait PAS perdre** : les appels arrêtés ne sont pas des légendes manquantes.
`LegendLifecycle` charge, juste après `init`, la légende de **toutes** les couches configurées — un
sur-ensemble de celles du thème actif. Le seam n'est utile qu'**après** le montage : changement de
style, changement de thème. C'est précisément ce que dit LG-18, et ce que la garde rend vrai.

📌 Le prédicat vit dans `legend.ts` (`isLegendInitialized`), pas sur `GeoLeaf.Legend` : c'est un
besoin **interne** au seam, et la surface publique d'un paquet publié ne s'élargit pas pour ça.
Le seam l'importe depuis l'implémentation, comme `lifecycle.ts` depuis B.28 — les **méthodes**, elles,
continuent de passer par la façade, d'où les deux imports.
...
| **`dependencies = ["geojson"]`, et surtout pas `"ui"`**     | Le module est dépilé **avant** le moteur de thème, qui est ce qui émet `app:ready` sur le chemin thémé. Ajouter `ui` le ferait dépiler après. ⚠️ Ce rang **ne porte plus seul** le montage : sur un profil **sans thème par défaut**, l'événement partait de l'`init()` de `ui` — dépilé AVANT ce module — et la légende restait vide, sans un mot (signalé sur 3.10.3). Le dévoilement attend désormais la fin du registre, et `whenAppReady()` monte à l'`init()` si l'application est déjà prête | `["geojson", "ui"]`                                                                                       |
...
| **Les deux canaux d'icône se mesurent par leur UNION**      | Chacun ne couvre qu'une part des graphies de `when.field`, et c'est voulu. Les éprouver un par un est ce qui a laissé une famille entière tomber entre les deux sans faire rougir un seul test                                                                                                                                                                                                                                                                                                      | N'ajouter qu'un test par canal : la garde `(c)` énumère les **12** graphies, une graphie neuve s'y ajoute |
...
⚠️ **La question de rang des dépendances se pose bien ici** :
...
🛑 **Et l'astuce ne tenait que sur le chemin thémé.** Le rang place ce module avant `theme-engine`,
mais APRÈS `ui` — or, sur un profil sans `defaultTheme`, c'est l'`init()` de `ui` qui dévoilait
l'application et émettait `app:ready`, avant que l'écouteur de la légende existe. Deux corrections,
indépendantes : le dévoilement « sans thème » est **armé** par `setupReveal()` et **libéré** par le
boot une fois `registry.init()` revenu (`app/init-reveal.ts`, `app/boot-core.ts`) ; et le cycle de
vie s'abonne par `whenAppReady()` (`kernel/shared/app-ready.ts`), qui monte aussitôt si
l'événement est déjà passé. Garde : `__tests__/app/boot-themeless-reveal.test.ts`, qui boote par le
vrai registre les trois cas (sans `themes`, `themes` sans défaut, défaut déclaré).

### Frontière `capabilities/` → `kernel/` (règle ESLint R.8)

| Import                                                                                       | Statut vis-à-vis de R.8                                                                                                                                                                                     |
| -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `kernel/shared/index.js` (`getAllLayerConfigs`, `registerLifecycleTeardown`, `whenAppReady`) | **Baril de médiation** — la légende est l'unique lecteur traversant de `getAllLayerConfigs`                                                                                                                 |
...
| §5 — CSS `css/geoleaf-legend.css`, « `@import` via `packages/core/src/css/geoleaf-main.css` » | Le fichier est `css/legend.css` et il est importé **par l'installeur**, ce qui le rend élaguable avec la capacité                                                                                |
