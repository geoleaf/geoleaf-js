...
⚠️ **Le seuil de deux profils est passé sous la barre puis repassé au-dessus — le même jour, le
10/08/2026.** Cette ligne a dit « aujourd'hui frôlé » du 27/07 au 10/08, puis « il est TOMBÉ » à
partir du retrait d'un profil du dépôt (`f218691e`, 04 h 15). Les deux énoncés étaient vrais à leur
date, et le second a cessé de l'être quinze heures plus tard : le profil, requalifié de « client » à
« démonstration synthétique », a été **restauré le soir même** (`818275503`). **Le seuil est donc de
nouveau atteint, et le sélecteur se rend dans les variantes livrées.** Le compte se mesure, il ne se
recopie pas — `ls profiles/ | grep -v '^schemas$' | grep -v '^_' | grep -v '\.'`, plus
`data.availableProfiles` du config racine livré, à relire dans chaque variante de `deploy/`.
⚠️ **Sous le seuil, c'est PS-04 qui s'applique : une dégradation volontaire, pas une erreur** — le
tenir pour un défaut du code est l'erreur que ce paragraphe existe pour empêcher. Éprouvé dans les
deux sens le 10/08 : dès qu'une seconde entrée est servie, le contrôle se monte à la bonne place
avec la bonne valeur, sans qu'une ligne de la capacité n'ait bougé.
...
| **Style aligné sur le sélecteur de style de couche**        | Le panneau porte déjà deux autres `<select>` natifs — celui du fond de carte et celui du style de couche ; un visiteur doit les lire comme un même type de contrôle. Les jetons de thème portent leurs propres replis, pour rester lisibles si une palette en omet un | Un style propre                                                                                               |
...
| `kernel/shared/index.js`                                   | **Baril de médiation** — nommé par la règle      |

Les quatre accès kernel de cette capacité passent donc par les quatre routes que la règle énumère
explicitement — et le seam est le cas d'école : c'est la forme prévue pour une dépendance inversée.
Le quatrième est arrivé avec la déclaration unique des clés de stockage (voir §Stockage écrit) :
passer par le baril est le geste que la règle DÉSIGNE, pas un contournement. Le reste passe par
`contracts/`, `api/` et `utils/` : `utils/log`, `utils/i18n`,
...
| « **8 profils** récoltés au build »                                             | Six profils de démonstration supprimés le 27/07/2026, puis un septième sorti du dépôt le 10/08/2026 — et restauré le soir même. Le compte se mesure, il ne se recopie pas, et le seuil de PS-04 est de nouveau atteint : voir §Configuration |
