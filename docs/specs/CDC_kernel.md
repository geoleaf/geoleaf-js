...
> extraite en `streamOgcApiFeatures` (une marche, dont `fetchOgcApiFeatures` devient l'accumulateur),
> le baril s'élargit de deux exports, et un module neuf annonce la troncature à l'utilisateur.
> **Aucune section de cette fiche ne bouge**, et c'est une relecture, pas une déduction :
>
> - **§K-06 / Les 13 sous-systèmes** — `geojson/` garde son baril et sa règle sur les types ;
>   l'élargissement du baril est le geste que R.8 **désigne** et il est motivé sur place.
> - **§Où va une fonctionnalité neuve** — le module de troncature répond « oui » à la question 1
>   (couplé aux internals du rendu : il consomme le membre `truncated` que le chargeur pose) et la
>   ligne « Kernel » de la table porte déjà `notify()`. Classement conforme, rien à écrire.
> - **§Séquence de boot** — aucun ordre ne bouge : le nouveau module n'a pas d'effet de bord à
>   l'import et n'est atteint que depuis un chargement de couche.
> - **§Contrat exposé** — mesuré, pas affirmé : aucun membre `GeoLeaf.*` n'apparaît ni ne change,
>   et le golden master `scripts/lib/namespace-surface.mjs` serait rouge, il ne l'est pas.
>
> La fiche du sujet est [`capacites/offline.md`](capacites/offline.md), estampillée sur le même
> commit avec le détail.
>
> ⚠️ **Ce que l'estampille de `349cf2a3a` couvrait — et pour une fois elle était MÉRITÉE, pas
> collatérale.** Le commit ajoute trois membres à la surface publique (`Layers.create`,
> `Layers.isVisible`, `Layers.isEnabled`) et deux interfaces à `layer-data.contract.ts` : il touche
> donc `contracts/`, que cette fiche décrit. Le §Contrats a été **réécrit** en conséquence — il
> porte désormais la leçon que l'irréversibilité des sous-chemins gouverne la FORME de ce qu'on y
> met, pas seulement le moment. Relues et inchangées : §Séquence de boot (aucun ordre ne bouge),
> §Les 13 sous-systèmes (`geojson/` garde son baril et sa règle sur les types), §Contrat exposé
> (elle ne recopie aucune liste de membres, elle cite la commande). Le compte de sous-chemins
> `contracts/` reste à **8** — deux interfaces ajoutées dans un fichier existant n'en créent pas.
>
> ⚠️ **Ce que l'estampille précédente couvrait**, conservé parce que la fiche en hérite : le commit
> `bc4516cac` ne touchait du cœur que **deux fichiers**, tous deux sous `capabilities/offline/db/` — le magasin d'images
> locales, qui gagne son adresse de retour et sa lecture, et le relais qui déclarait trop large le
> statut de téléversement. Mesuré : `git show --name-only <sha> -- packages/core/src/kernel` rend
> **zéro** fichier. Sa fiche est [`capacites/offline.md`](capacites/offline.md), pas celle-ci, et
> aucun membre `GeoLeaf.*` n'apparaît — ce qui n'est pas une affirmation mais une **mesure** : le
> golden master de la surface publique (`scripts/lib/namespace-surface.mjs`) serait rouge, et il ne
> l'est pas. Les sections que cette zone pourrait périmer ont été relues — §Séquence de boot,
> §Les 13 sous-systèmes, §Contrat exposé — et aucune ne bouge, **par construction** : cette fiche
> ne recopie ni signature ni liste de membres.
>
> ⚠️ **Ce que l'estampille d'avant elle couvrait**, conservé pour la même raison : les deux
> commits qui l'ont rendue nécessaire touchent le **contrat de synchronisation** (deux champs optionnels, puis un
> membre d'union `authRequired`), le **contrat de bus** (un événement du seam connecteur), la
> **façade `Storage`** (deux membres) et la capacité `offline` — dont la fiche est
> [`capacites/offline.md`](capacites/offline.md), pas celle-ci. Les sections de CETTE fiche que
> ces trois zones pourraient périmer ont été relues : §Séquence de boot (B8 inchangé), §Les 13
> sous-systèmes (`shared/` et `storage/` inchangés), §Contrat exposé et §Contrats. Aucune ne
> bouge, **par construction plutôt que par chance** : cette fiche ne recopie ni signature ni
> liste de membres, elle cite la commande qui les imprime — et ajouter à `contracts/` est ce
> qu'elle décrit comme toujours permis (« on en ajoute quand on veut ; on n'en retire jamais »).
> Le reste de la fiche est repris de la vérification du **01/09/2026** (`70cf3d31`) et n'a **pas**
> été re-mesuré.
...
| K-04 | Carte, conteneur, thème de carte, **cycle de vie du registre**               | config `map` du profil                                        | carte montée, `geoleaf:map:ready` ; registre à clé (`init`/`destroy`/`hasMap`/`listMaps`) et **déplacement d'une carte vivante** (`isAttached`/`reattach`, cf. **ADR-15**) — un seul registre, celui de `Core` : `GeoLeaf.getMap`/`getAllMaps` y délèguent depuis la 3.1.0 ; un `destroy` pendant le boot l'**interrompt** (`geoleaf:boot:aborted`, `reason: "destroyed"`) au lieu de le faire échouer — le registre retient la carte retirée                                                                                                                                                                                                                                                                                       | `kernel/map/`                                             |
| K-05 | Fonds de carte (raster, vectoriel, WMS/WMTS, hillshade, image géoréférencée) | `config/core/basemaps.json`                                   | sélecteur de fonds, `geoleaf:basemap:change` ; crédit du fond affiché par le contrôle d'attribution de MapLibre — celui d'un fond vectoriel compris, porté par la transformation de style (3.4.0) ; un basculement qui remplace le style y porte les couches de l'adaptateur et celles que du code extérieur lui a DÉCLARÉES (`IMapAdapter.declareOwnedStyleIds`, 3.10.0) — les étiquettes, elles, se reconstruisent sur `style.load`                                                                                                                                                                                                                                                                                               | `kernel/basemaps/`                                        |
...
| K-08 | Coquille UI — panneau desktop, barre mobile, composants partagés             | `config/core/ui.json`                                         | DOM de la coquille, `geoleaf:toolbar:action` ; dans la barre mobile, le groupe de filtre existe dès que `modules.filter.enabled` n'est pas `false`, quels que soient les boutons fixes éteints (géolocalisation, thèmes, couches) — ancré sur un voisin nommé, **juste avant Couches**, sinon après le dernier bouton fixe (3.10.5 ; jusqu'en 3.10.4, inséré au 7ᵉ bouton, il disparaissait dès qu'un seul manquait) ; dans le panneau desktop, les trois onglets intégrés prennent leurs titres au dictionnaire (`sheet.title.filters` / `.layers` / `.legend`, les clés de la feuille mobile), un titre de profil l'emportant (3.10.6 ; jusqu'en 3.10.5, « Filtres », « Couches », « Legende » en dur, quelle que soit la langue) | `kernel/ui/`                                              |
...
| K-18 | i18n du core                                                                 | `?lang=` → `localStorage['gl-lang']` → `ui.language` → `"fr"` | libellés traduits, `GeoLeaf.I18n.getActiveLang()`. La langue se fixe au boot (`SharedModule` #1, après la config, le profil et `beforeBoot`). C'est aussi le boot qui publie `<html lang>`, sauf si `ui.syncDocumentLang` vaut `false`. Avant le boot, un libellé suit la configuration du moment et n'écrit rien. Aucun libellé ne se résout à l'import d'un module                                                                                                                                                                                                                                                                                                                                                                | `lang/`, `utils/i18n/`                                    |
...
sous-couches est décidé **une seule fois, à la création**, et aucune écriture ne le rejoue. Le
sur-rendu est donc la seule option sûre, et il ne coûte plus rien puisque chaque sous-couche est
confinée.

⚠️ **Cette phrase prouvait l'invariant par « `updateLayerData()` ne fait que `source.setData()` », et
cette preuve est périmée depuis R6.** Une mutation unitaire atteint désormais le moteur sous forme
de **diff** (`IMapAdapter.applyDataDiff` → `GeoJSONSource.updateData`) quand la couche est
adressable par id. **L'invariant, lui, tient toujours** — et pour une raison plus forte que
l'ancienne : `updateData` ne touche que les features d'une source, jamais les couches. Ce qui a
changé n'est pas ce qui est vrai, c'est ce qui le démontre.

📌 **Corollaire qui était DÉJÀ vrai et que personne n'avait écrit** : une écriture peut introduire
une famille de géométrie que le jeu de sous-couches ne couvre pas. C'était le cas avec `setData`
comme ça l'est avec un diff — le repli aux trois familles sur données d'amorce vides existe
précisément pour ça. Le diff ne crée pas ce trou, il rend seulement plus visible qu'il n'a jamais
été comblé.
...
```
beginBoot()                        (surveillance : watchdog armé, échecs signalés ; état « prêt » remis à zéro)
...
releaseThemelessReveal()           (profil SANS thème par défaut : le dévoilement, une fois tous les init() passés)
```

🛑 **`geoleaf:app:ready` part APRÈS le dernier `init()` du registre — sur les deux chemins.** Les
capacités qui montent sur cet événement (légende, échelle, coordonnées, filtre, sélecteur de thème)
dépendent de `["geojson"]` seul : le tri les dépile **après `ui`** et avant `theme-engine`.

- **Profil thémé** : `setupReveal()` (étape #23 de `UIModule.init()`) écoute
  `geoleaf:theme:applied`, que `theme-engine`, dernier dépilé, émet. C'est aussi le cas d'un
  `themes` **sans** `defaultTheme` : il démarre sur `themes[0]`.
- **Le thème de départ a UN résolveur**, `kernel/config/default-theme.ts`, sans import (le lire
  depuis `kernel/themes` fermerait un cycle avec `kernel/geojson`). La règle : le défaut déclaré
  s'il nomme un thème de la liste (`defaultTheme`, puis la clé historique `config.defautTheme`),
  sinon le premier thème qui a un id, sinon `null`. Trois lecteurs répondaient chacun à leur façon :
    - le chargeur GeoJSON et le dévoilement disaient « aucun » sans clé déclarée, pendant que le
      chargeur de thèmes appliquait `themes[0]` ;
    - le chargeur GeoJSON chargeait alors toutes les couches en arrière-plan ;
    - mesuré sur tourism : 19 à 21 couches visibles pour un thème de 7, un nombre différent à chaque
      chargement.
- **Un profil modulaire sans `themes` ne déclare aucun thème** : `theme-engine` ne consulte plus
  le chargeur de thèmes pour lui. Le chargeur serait retombé sur le `profiles/<id>/themes.json`
  hérité (404, avec un nouvel essai à 1 s), et le dévoilement attend ce module : 1,1 à 2,1 s de
  voile, mesurés. Un profil hérité passe toujours par le chargeur. La branche 4 de
  `ThemeLoader.loadThemesConfig` (K-07) est inchangée.
- **Profil sans thème par défaut** : `setupReveal()` **arme** le dévoilement, et `boot-core` le
  **libère** une fois `registry.init()` revenu, sauf sur une carte que l'hôte a détruite entre-temps.
- **Ce qui était faux** : ce dévoilement partait de l'`init()` de `ui`, donc AVANT les capacités.
  Aucune ne montait, et rien n'était journalisé (signalé par un intégrateur sur 3.10.3).
    - Une micro-tâche n'aurait pas suffi : le registre attend chaque `init()`, donc elle passe avant
      que la boucle reprenne.
    - En plus, les cycles de vie s'abonnent par `whenAppReady()` (`kernel/shared/app-ready.ts`, dans
      le baril de médiation). Un `init()` tardif y monte aussitôt au lieu d'attendre un événement déjà
      passé.
    - Le drapeau retombe au début de chaque boot et à `Core.destroy()`.
- **Garde** : `packages/core/__tests__/app/boot-themeless-reveal.test.ts` boote les trois cas par
  le vrai registre. Le cas de l'hôte se rejoue dans un vrai navigateur :
  `e2e/cfg-c3-basemaps-themes-taxonomy.spec.js`.
...
3. Si `profile.bundleFile` est déclaré → **un seul fetch** de `profile-bundle.json` (portant lui
   aussi le jeton de cache, cf. ci-dessous — il a été requêté NU jusqu'au 06/09/2026), pré-fusionné
...
**Le jeton de cache des étapes 2 à 4.** Chacune de ces requêtes porte un `?t=<jeton>`, résolu
**une fois** dans `loadActiveProfileResources()` puis passé de main en main — huit sites
d'interpolation, le bundle compris. Sa valeur :

| Situation                      | Jeton                                                                 |
| ------------------------------ | --------------------------------------------------------------------- |
| `debug: true`                  | `Date.now()` — défait tout cache à chaque chargement, et c'est le but |
| `data.profileVersion` déclarée | cette valeur, **percent-encodée**                                     |
| ni l'un ni l'autre             | `0`                                                                   |

**Dans un DÉPLOYÉ, la clé n'est pas à déclarer : le build l'écrit.** `build-deploy.cjs` pose
l'empreinte de contenu de l'arbre `profiles/` copié (`scripts/lib/profile-fingerprint.cjs`),
selon le patron de `data.availableProfiles` — absente de la source, présente dans l'artefact.
C'est un condensat, pas un identifiant de build : deux constructions à sources identiques
rendent le même jeton, donc un déploiement qui ne change rien ne fait rien re-télécharger.

🛑 **Le fichier qui PORTE le jeton ne peut pas être invalidé par lui.** L'empreinte est écrite
dans `profiles/geoleaf.config.json`, et ce fichier est requêté **sans** `?t=` — il est le
bootstrap. Épinglé par un `max-age` long, il ne livre jamais la nouvelle empreinte et défait le
mécanisme entier. C'est pourquoi les deux recettes serveur émises avec le livrable le déclarent
`no-cache`, et pourquoi **`SC-05`** refuse une recette qui ne le fait pas : le fait ne peut pas
être gaté chez l'intégrateur, il voyage donc avec l'artefact.

⚠️ **Le jeton est résolu AVANT que `profile.json` soit requêté** — l'étape 2. Une valeur écrite
dans `profile.json` arriverait donc trop tard pour lui comme pour tout ce qui suit : la clé se
fournit dans la configuration racine, ou se pousse dans le magasin par l'application intégrante
avant le boot. C'est un ordre, pas une préférence.

🛑 **Le défaut `0` est une URL FIXE, et c'est un piège chez qui sert le déployé derrière un
`Cache-Control: max-age` long** : le profil et toutes ses annexes restent alors épinglés dans un
navigateur déjà ouvert pour la durée de l'en-tête — ni un vidage de cache serveur ni une
régénération n'y changent quoi que ce soit. `debug: true` n'est pas la réponse : c'est un mode de
mise au point qui défait aussi le cache du service worker à chaque chargement.
...
⚠️ **L'`order` est celui des SECTIONS. Une couche n'en a pas** : dans une section, les couches se
trient par `zIndex` décroissant (`geojson/layers/integration.ts`), pour que la liste reflète
l'empilement sur la carte. `layers.schema.json` a promis un `order` par entrée, et un
`defaultVisible`, sans qu'aucun code ne les lise ; les deux en sont retirés depuis le 25/09/2026.
Une entrée d'index porte `id`, `configFile`, `layerManagerId` et `label`. Avec un `configFile`,
son `label` remplace celui de la config (`_buildEnrichedProfile`) ; sans `configFile`, l'entrée
est la couche elle-même.
...
### Une couche hors du panneau — `showInLayerManager: false`

Une couche qui déclare `showInLayerManager: false` (fichier de couche, ou définition passée à
`Layers.create`) n'a **aucune ligne** dans le panneau COUCHES. Elle se charge, suit son thème et
garde sa légende : la clé retire une bascule, pas une couche.

🛑 **Le filtre est posé sur les DEUX producteurs, et c'est l'invariant.** Le panneau est nourri
par `registerWithLayerManager` (couches chargées, `state.layers`) **et** par
`populateLayerManagerWithAllConfigs` (couches configurées, rejoué à chaque
`geoleaf:theme:applied`). Les deux versent dans `_registerGeoJsonLayer`, un puits idempotent qui
**ne retire jamais** une ligne : un filtre sur un seul chemin laisserait l'autre remettre la couche,
au premier changement de thème. Le second chemin lit une **projection en liste blanche**
(`loadAllLayersConfigsForLayerManager`) : une clé qu'elle ne recopie pas n'existe pas pour lui.

⚠️ **Ni le store, ni le puits.** Le store des configurations n'est pas filtré : la capacité
`legend` l'itère pour charger la légende de toute couche. Le filtre précède la création de la
section, donc une section **non déclarée** qui ne contiendrait que de telles couches n'est pas
créée ; une section **déclarée** dans `layerManagerConfig` naît vide et garde son titre, sans ligne.

### Une couche où l'on cherche — `searchable`

Une couche qui déclare `searchable: { fields: [...] }` est lue par `GeoLeaf.Layers.search` : sur
ces champs et sur l'identifiant de chaque entité, avec la règle de correspondance du filtre
(accents, casse et ordre des mots ignorés — `utils/general/normalize-text.ts`, une seule règle
pour les deux). Une couche sans la clé n'est jamais cherchée. `GeoLeaf.Layers.focus` cadre ensuite
l'entité trouvée et pose son état `selected`, que les sous-couches peignent désormais — cercle,
ligne et contour, à la création comme après tout re-style.

🛑 **La recherche lit le MAGASIN MÉMOIRE des couches, pas le stockage de l'appareil.** Il contient
déjà ce qui compte hors réseau — une couche déclarée hors ligne y est relue du stockage avant tout
réseau — et les couches statiques, que le stockage de l'appareil ne tient jamais. Deux trous, par
construction : une couche jamais chargée (`active: false`) n'a rien en mémoire, et une couche de
tuiles vectorielles ne garde aucune entité. L'index est une seconde copie du magasin, **invalidée
chez ses écrivains** (écriture d'une collection entière, diff unitaire, `patchFeature` silencieux)
et jamais devinée.

⚠️ **La clé s'appelle `searchable` et non `search`, pour un motif.** `geojson-filter.ts` lisait
encore un `search.enabled` que le schéma avait retiré, et une clé `search` aurait rallumé ce chemin
en silence. Ce lecteur est retiré en 3.10.4 : le filtre traite désormais toute géométrie de la même
façon, lignes comprises, et un bloc `search` résiduel n'a plus d'effet. Le nom `searchable` reste,
...
outre en trois passes, une par famille : une couche `polyline` déclarée, `mixed` ou `unknown`
n'entrait dans aucune. Et `setVisibleSubset` n'avait aucun effet sur une ligne. Preuve sur le
bundle livré : `e2e/64-filter-reaches-line-layers.spec.js`.
...
| Membre                | Rôle                                                                                         |
| --------------------- | -------------------------------------------------------------------------------------------- |
| `registerPanelPane()` | déclare `{ id, labelKey, selector, order?, icon?, onOpen? }` ; idempotent par `id`           |
...
🛑 **`.gl-rp-theme-separator` est une ancre de LAYOUT, inconditionnelle — pas un ornement du
bouton de thème.** Il porte la seule marge `auto` du bandeau, et c'est elle qui **pousse la pile
d'icônes contre le bord bas**. Il est de plus le repère d'insertion du variant `"tab"`
(`_insertTabButton`) et de `syncRegisteredPanes`, qui ne regardent jamais le bouton. Le bouton,
lui, est gaté par `ui.showPanelThemeToggle` (défaut `true`, routé par `DesktopPanelOptions
.showThemeToggle`). ⚠️ Conséquence pour tout nettoyage futur : le séparateur **survit** au
retrait du bouton, et le passer en `display: none` ne le neutralise pas — il cesserait de
générer une boîte, donc onglets et icônes se tasseraient ensemble en haut.

⚠️ **Cette règle a été énoncée le 05/09 sur une prémisse qui n'existe plus** : « `.gl-rp-tabs` est
une colonne flex **avec** `justify-content: center`, et cette marge annule ce centrage ». Le
centrage était lui-même le défaut — un débordement en colonne centrée se répartit à parts **égales
aux deux bouts**, si bien qu'un bandeau trop haut pour le viewport perdait son premier onglet ET sa
dernière icône, coupés par `body { overflow: hidden }` sans ascenseur. Le bandeau est en
`flex-start` depuis le 06/09. Le rôle du séparateur survit ; la raison qu'on en donnait, non.

🛑 **Les onglets du bandeau sont des ICÔNES, avec repli texte.** Six onglets en texte vertical
(`writing-mode: vertical-rl`, `white-space: nowrap`, donc incompressibles) débordaient un viewport
d'ordinateur portable — « PARTAGER MA POSITION » à lui seul valait un cinquième de l'écran. Une
icône mesure 28 px quelle que soit la langue. Le libellé **reste le nom accessible** (`aria-label`
et `title`) : le texte ÉTAIT le nom du bouton, et six boutons anonymes dans un `tablist` sont ce
que le scan axe de l'E2E dit tout haut. ⚠️ Un pane qui ne déclare **pas** d'`icon` garde un onglet
texte — la couture est publique, et faire disparaître le pane d'un tiers derrière un carré vide
serait pire que le mot long qui a motivé la bascule.
...
🛑 **Et cette irréversibilité gouverne la FORME de ce qu'on y met, pas seulement le moment.** Le
cas mesuré, le 04/09/2026, sur `layer-data.contract.ts` : l'état de visibilité d'une couche porte
six champs, dont `current` (ce qui est peint) et `logicalState` (ce que l'utilisateur a demandé),
qui divergent dès qu'une plage de zoom s'applique. Publier l'objet aurait été le geste évident et
le mauvais — il aurait gravé, sans retour possible, une forme où lire le mauvais champ pour piloter
une bascule est un défaut déjà commis une fois. Ce qui est entré au contrat, ce sont **deux
accesseurs nommés** — `isVisible` et `isEnabled` — qui rendent la confusion indicible au lieu de
seulement la déconseiller. La question à poser avant d'ajouter un sous-chemin n'est donc pas
« a-t-on besoin de cette donnée ? » mais « **quelle forme accepte-t-on de ne jamais pouvoir
reprendre ?** »

📌 **La question s'est reposée le 05/09/2026, et la réponse a de nouveau été « pas l'objet ».** Deux
besoins restaient sur ce même état : savoir QUI a décidé, et écrire en nommant une source. Sont
entrés un **troisième accesseur** (`isUserOverridden`) et une écriture dont le paramètre de source
est **obligatoire** — optionnel, elle aurait dupliqué `GeoJSON.showLayer`, et deux portes vers la
même pièce donnent deux réponses à la même question. ⚠️ Le type des sources est une **union et non
un enum**, parce que cette surface est type-pure et ne peut pas exporter de valeur : la contrainte
a rendu le résultat plus petit que le geste évident, ce qui est le sens dans lequel elle est utile.
**L'objet à six champs n'est toujours pas public**, et cette ligne compte désormais deux
applications de la même règle plutôt qu'une.

📌 **Troisième application, le 20/09/2026, et elle a servi DEUX demandes par le même geste.**
① Un **quatrième accesseur**, `getVisibilitySource`, rend la source elle-même — `isUserOverridden`
ne répond que « pas l'utilisateur », ce qui rend un thème indiscernable d'un seuil de zoom, et un
appelant qui reconstitue la valeur à partir du booléen écrit la même chose pour les deux. ⚠️ Il
rend `null` et non une source par défaut pour une couche inconnue : là où les booléens voisins
rendent `false` au sens de « rien à peindre », **tout nom de source affirme que quelqu'un a
décidé** — inventer `"system"` serait porter cette affirmation sans le fait. ⚠️ Sa contrepartie
honnête est écrite au contrat : la source enregistrée **retarde sur une écriture refusée**, le
manager ne l'écrivant que si le changement physique aboutit. Le contrat dit donc de la lire comme
« qui a décidé de ce qui est affiché », jamais « qui a demandé en dernier ».
② `getStyle` rend une **projection à deux champs** (`id`, `label`) là où la demande d'aval visait
à typer `GeoJSON.getLayerById`. Le refus est le même qu'en 08 : `currentStyle` n'a pas une forme
mais **deux**, selon la main qui a écrit en dernier — peinture aplatie sans `id` ni `label` quand
un style s'applique, document complet quand le sélecteur le restaure par-dessus —, et son `label`
est lui-même polymorphe. Typer l'entrée de couche aurait gravé cette instabilité dans le contrat,
sans retour. **La peinture reste hors contrat**, et le motif est écrit sur place plutôt que laissé
à déduire : c'est exactement la part dont la forme n'est pas stable.

🛑 **Ce que ces trois applications ont en commun, et qui est la règle utile** : dans les trois cas
la demande portait sur une DONNÉE, et la réponse a porté sur une FORME. Aucune n'a été refusée ;
aucune n'a été servie telle que formulée.

📌 **Quatrième application, le 22/09/2026 — et elle n'a rien publié de neuf côté API.** `getStyle`
ne servait pas le seul site qui la motivait : un hôte ÉCRIVAIT dans `currentStyle.label` pour
qu'une couche au fichier de style absent affiche ses libellés. La demande hésitait entre un chemin
public d'écriture et un bloc de configuration honoré par le chargeur. La réponse est le bloc : la
définition de couche porte déjà ses libellés (`labels`, même objet que le `label` d'un style), le
chargeur le lisait comme DÉCLENCHEUR, et tous les lecteurs suivants regardaient le style seul. Un
résolveur unique — `kernel/geojson/layer-labels.ts`, ré-exporté par le baril `geojson` pour la
capacité (R.8) — répond à tous : le `label` OBJET du style s'il existe, sinon le bloc de la
définition. **Écrire dans `currentStyle` reste hors contrat** : on n'a publié ni la forme instable
ni un accès en écriture à elle, seulement complété une clé déclarative qui existait à moitié.

📌 **Cinquième application, le 25/09/2026 — une forme de RÉSULTAT, pas d'entité.**
`Layers.search` rend des `FeatureMatch` (`label`, `lat`, `lng`, `bounds?`, `layerId`, `featureId`),
jamais l'entité elle-même : rendre la `Feature` aurait gravé au contrat la forme de ce que le
magasin garde, et offert à l'appelant une référence vivante sur lui. La projection est celle d'un
résultat de géocodage, pour qu'un fournisseur de recherche la rende telle quelle ; qui veut
l'entité la relit par `getFeatureById(layerId, featureId)`. `focus` rend un booléen — `false`
quand la couche ne tient pas l'entité — et non un état de caméra, qui appartient à l'adaptateur.
...
- **`GeoLeafRawEventMap`** — **quatre** clés y vivent, et volontairement : `geoleaf:toolbar:action`
  ...
  `Error` — ou `"destroyed"` quand `Core.destroy()` a retiré la carte pendant le boot). Aucune ne survit à la sérialisation. Les placer dans la première carte aurait rendu
...
| **ADR-15** | `Core.reattach()` déplace le **conteneur de carte**, et **rien d'autre** — les panneaux de la coquille ne suivent pas                                                                                                                                                                                                                                                                                                                                         | deux contraintes, l'une du moteur et l'autre d'architecture. ① MapLibre **mémorise l'élément passé à la construction** : déplacer les enfants du conteneur au lieu du conteneur lui-même laisserait `map.getContainer()` pointer l'ancien nœud, et toute mesure ultérieure fausse — c'est exactement le contournement que l'aval avait dû écrire faute de poignée sur la carte. ② `#gl-right-panel` et ses voisins vivent dans `glMain`, hors du conteneur : les faire suivre rendrait l'API **dépendante du DOM de la coquille**, le couplage même qu'elle sert à supprimer. Le remontage est donc du ressort de l'hôte, et il compte **deux recettes, une par surface** — `UI.destroyDesktopPanel()` → `initDesktopPanel()` → `activateDesktopPanel()` pour le panneau, `UI.destroyMobileToolbar()` → `initMobileToolbar()` pour la pilule ; le TSDoc dit les deux. ⚠️ **Il n'en disait qu'une jusqu'au 07/09/2026**, alors qu'il écrivait « `#gl-right-panel` **et ses voisins** » : les voisins sont quatre nœuds appendus par `initMobileToolbar`, pour lesquels il n'existait ni destroy ni garde d'idempotence. Une recette incomplète qui se présente comme complète coûte plus qu'une absence — l'aval a suivi la moitié fournie, cassé l'autre, et abandonné `reattach` pour un conteneur persistant. 🛑 **Et les deux destroys se lancent AVANT que la coquille tombe** : chacun rend des nœuds qu'il avait déplacés (la feuille mobile détient le panneau de filtres, la légende et le gestionnaire de couches quand elle est ouverte), et il ne peut les rendre qu'à un parent qui existe encore — sans cette phrase l'intégrateur reproduit le patch aval | faire suivre les panneaux — couplage de l'API au DOM de la coquille ; ou déplacer les enfants du conteneur — bug de `getContainer()` importé tel quel                                                                                                                    |
...
**Le seul crochet qu'un plugin pose sur le global pour le kernel, et il accepte une promesse
depuis 3.4.0.** `__GEOLEAF_WORKER_HEADERS_HOOK__` est lu par `kernel/geojson/worker-manager.ts`
juste avant de poster une requête à l'ouvrier GeoJSON ; un plugin — le connecteur — y rend les
en-têtes d'authentification de l'URL, sans que le kernel l'importe. Le kernel l'appelle avec
`{ acceptsPromise: true }` en second argument : une réponse synchrone part aussitôt, comme
avant ; une promesse fait partir la requête à son règlement, si l'entrée en attente est
toujours la même (sinon elle est rejetée, jamais laissée pendante) ; une exception ou un rejet
la laisse partir sans en-têtes. ⚠️ **Un crochet ne rend une promesse que s'il l'a lue dans cet
argument** : un kernel antérieur la passerait telle quelle à `postMessage`, qui ne clone pas une
...
| ---------- | ------------------ | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **4.3.22** | 26 septembre 2026  | Claude Opus 5.5 | **§Séquence de boot dit désormais QUAND part `geoleaf:app:ready` : après le dernier `init()` du registre, sur les deux chemins.** Sur un profil sans thème par défaut, `setupReveal()` dévoilait l'application depuis l'`init()` de `ui` — avant les capacités que le tri dépile ensuite (légende, échelle, coordonnées, filtre, sélecteur de thème), et aucune ne montait, sans un mot. Signalé par un intégrateur sur 3.10.3. ✅ **Ce qui est entré** (§Séquence de boot, phase B) : le dévoilement « sans thème » armé puis libéré par `boot-core` après `registry.init()` ; `whenAppReady()` dans le baril `kernel/shared/` ; le drapeau remis à zéro au début du boot et à `Core.destroy()` ; le résolveur unique du thème de départ (`kernel/config/default-theme.ts` — un `themes` sans défaut démarre sur `themes[0]` chez les trois lecteurs, la clé historique compte partout) ; `theme-engine` ne cherche plus de `themes.json` pour un profil modulaire sans thèmes ; les gardes (vitest par le vrai registre, E2E du cas de l'hôte, couches chargées comprises). Core 3.10.6. ✅ **Relu et jugé INTACT** : l'ordre du registre (aucune dépendance ne bouge) ; « le voile, que seul `revealApp()` masque et que seul `UIModule` arme » ; ADR-08 (le registre reste le seul orchestrateur du runtime — la libération suit son `await`, elle ne le contourne pas) ; §Contrats (aucun export public neuf ; `whenAppReady` est interne au baril de médiation). 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **4.3.21** | 26 septembre 2026  | Claude Opus 5.5 | **K-08 dit d'où les onglets du panneau desktop tirent leurs titres.** Sans titre de profil, `initDesktopPanel` posait « Filtres », « Couches », « Legende » en dur : un hôte en anglais avait tout traduit sauf ces trois onglets, et seulement au-delà de 1440 px, tandis que la feuille mobile lisait déjà `sheet.title.*`. ✅ **Ce qui est entré** (K-08) : les mêmes clés que la feuille, un titre de profil gardant la priorité, core 3.10.6. ✅ **Relu et jugé INTACT** : §Séquence de boot — le panneau se construit dans `initUIPanels`, après `initI18n()`, donc lire le libellé à l'appel suffit et aucun changement de langue ne survient en cours de page ; §Le registre de panes dit encore vrai : « le desktop lit … trois titres, le sheet lit `getDefaultSheetTitles()` » — les défauts des deux côtés viennent désormais des mêmes clés, les formes restent distinctes. 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **4.3.20** | 26 septembre 2026  | Claude Opus 5.5 | **K-08 dit où vit le groupe de filtre de la barre mobile, et qu'il existe quelle que soit la barre.** Le groupe était inséré quand la boucle des boutons fixes atteignait l'index 6. La liste compte quatre boutons permanents et trois conditionnels (géolocalisation, thèmes, couches) : un seul éteint, et l'index 6 n'était jamais atteint — le bouton de filtre manquait sans un mot, panneau actif. Le rang avait déjà dérivé une fois : posé pour « après Proximité » quand la recherche était encore dans la liste, il était devenu « après Thèmes » à son départ. ✅ **Ce qui est entré** (K-08) : l'ancrage avant Couches, sinon après le dernier bouton fixe, core 3.10.5 ; une barre complète garde sa disposition. ✅ **Relu et jugé INTACT** : §Séquence de boot (la barre se construit au même moment, seule la position d'insertion change) ; §Contrats (aucun export touché, `createToolbarDom` garde sa signature). 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **4.3.19** | 26 septembre 2026  | Claude Opus 5.5 | **Le filtre de couche a sa sous-section, et le motif du nom `searchable` est dit au passé.** La fiche disait que `geojson-filter.ts` lit « encore » `search.enabled`. C'était vrai, et c'était le défaut : ce lecteur soustrayait toute couche de lignes au filtre, alors que la clé avait quitté le schéma le 08/07. Il est retiré en 3.10.4. La sous-section neuve dit ce que le seam fait maintenant (une passe, toute géométrie, familles de types repliées), ce qu'il faisait jusqu'en 3.10.3, et sa limite sur les tuiles vectorielles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **4.3.18** | 25 septembre 2026  | Claude Opus 5.5 | **§`layerManagerConfig.sections[]` dit désormais qu'une COUCHE n'a pas d'`order`.** Le paragraphe décrivait l'`order` des sections, et il était juste. Son silence sur les couches a laissé vivre une promesse du schéma : `layers.schema.json` acceptait un `order` et un `defaultVisible` par entrée, sans qu'aucun code ne les lise. Dans une section, le tri se fait par `zIndex` décroissant. ✅ **Ce qui est entré** : ce tri, le retrait des deux clés du schéma, et ce qu'une entrée d'index transporte réellement (`label` remplace désormais celui de la config, core 3.10.2). ✅ **Relu et jugé INTACT** : l'invariant d'ordre d'enregistrement du même paragraphe ; K-06 ; §Séquence de boot. 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **4.3.17** | 25 septembre 2026  | Claude Opus 5.5 | **K-18 dit désormais QUAND la langue se fixe, et qui écrit `<html lang>`.** La ligne donnait l'ordre de résolution. Elle ne disait pas le moment, et c'est le moment qui était faux dans le code. Le titre du gestionnaire de couches et le texte du branding se traduisaient à l'IMPORT de leur module. Le premier des deux lançait la première résolution de la page, sur une configuration vide, et figeait la langue par défaut. Cette résolution publiait aussi l'attribut avant qu'un hôte ait pu poser `ui.syncDocumentLang: false`. ✅ **Ce qui est entré** (K-18) : la langue se fixe au boot (`SharedModule` #1, après la config, le profil et `beforeBoot`), et c'est ce seul appel qui publie l'attribut. Avant le boot, un libellé suit la configuration du moment et n'écrit rien. Aucun libellé ne se résout à l'import. ✅ **Relu et jugé INTACT** : §Séquence de boot (l'ordre des modules ne bouge pas, `shared` #1 appelait déjà `initI18n()`) ; §Contrats (aucun export ni membre neuf, `Config.set` gagne la garde de `get` et un TSDoc) ; §Les 13 sous-systèmes (aucun `index.ts` touché). 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **4.3.16** | 25 septembre 2026  | Claude Opus 5.5 | **Re-vérification de TROIS commits (`56909c7b1`, `bb52e02c4`, `f1ef40e1f`) et de leur correctif de clôture — un membre de recherche, un de cadrage, deux au stockage.** Deux faits manquaient et sont écrits : la clé de couche `searchable` (§Configuration — ce que la recherche lit, ses deux trous, pourquoi pas `search`) et la cinquième application de la doctrine de forme (§Contrats — `FeatureMatch` est une projection de résultat, pas l'entité). Relues sans changement : §Séquence de boot (la façade `Storage` gagne `resolveOptions` et `preflight`, qui rendent `null` sans moteur — B8 inchangé), §Les 13 sous-systèmes, K-06 (la sélection peinte s'ajoute au trait des sous-couches existantes, aucune n'est créée), §Dépendances et frontières (la règle de correspondance vit sous `utils/`, ouvert aux capacités par R.8). 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **4.3.15** | 24 septembre 2026  | Claude Opus 5.5 | **Re-vérification d'UN commit (`f744a8ac9`) : les couches posées hors de l'adaptateur traversent un changement de style — un membre neuf au contrat, core 3.10.0.** ✅ **Ce qui est entré** : K-05 dit désormais ce qu'un basculement qui remplace le style porte — les couches de l'adaptateur et celles que du code extérieur lui a DÉCLARÉES (`IMapAdapter.declareOwnedStyleIds`), les étiquettes se reconstruisant sur `style.load`. 🛑 **La forme gravée, au sens du §Contrats** (« quelle forme accepte-t-on de ne jamais pouvoir reprendre ? ») : un membre OPTIONNEL, une déclaration par clé de déclarant qu'un second appel REMPLACE et que `null` retire — ni accumulation qu'un retrait devrait défaire id par id, ni préfixe d'identifiant reconnu, que la transformation s'interdit ; les ids se lisent quand elle S'EXÉCUTE, comme le reste de ce qu'elle porte depuis 4.3.14. ✅ **Relu et jugé INTACT** : le §Contrats dérive des `exports` et n'énumère aucun membre ; aucun `index.ts` ni `global.d.ts` touché (le type neuf `DeclaredStyleIds` passe par le `export type *` de `map-adapter.contract.ts`, sous-chemin déjà exporté) ; la §Séquence de boot ne bouge pas — l'activation silencieuse du fond par défaut suit le même chemin qu'avant, seules les étiquettes y survivent désormais. 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **4.3.14** | 24 septembre 2026  | Claude Opus 5.5 | **Re-vérification d'UN commit (`e84b8bff4`) : un fond vectoriel appliqué au boot effaçait les couches créées pendant le téléchargement de son style, et le panneau latéral ne rangeait le gestionnaire de couches que s'il existait déjà.** ✅ **Ce qui a été relu et jugé INTACT, contre le code** : K-05 dit le crédit d'un fond vectoriel « porté par la transformation de style » — toujours vrai, la transformation le pose avant la fusion, dès le premier chargement ; le §registre de panes dit que l'hôte déplace le nœud et le remet à la fermeture, « le geste déjà appliqué à `.gl-map-legend` et `.gl-layer-manager` » — plus vrai qu'avant, les trois passant désormais par le même geste, y compris quand le nœud arrive après l'activation ; ADR-15 et son triplet `destroyDesktopPanel()` → `initDesktopPanel()` → `activateDesktopPanel()` tiennent ; le §Contrats **dérive** des `exports` et n'énumère aucun membre, donc le TSDoc réécrit d'`IMapAdapter.buildStyleChangeTransform` (plus aucun `null` promis) n'y fait pas trou ; aucun `index.ts` ni `global.d.ts` touché. 📌 **Cette fiche ne disait nulle part QUAND la transformation lit ce que GeoLeaf possède** — et c'est ce silence qui a laissé vivre le défaut : MapLibre n'exécute le rappel qu'à l'arrivée d'un style donné par URL. Le fait vit désormais dans l'en-tête de `maplibre-style-transform.ts` et dans le contrat, là où le prochain lecteur du mécanisme regarde ; rien n'est ajouté ici. 🖐 Re-vérification du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                  |
| **4.3.13** | 24 septembre 2026  | Claude Opus 5.5 | **Un ajout public en 3.9.0 : une couche peut rester hors du panneau COUCHES.** ✅ **Ce qui est entré** (§`layerManagerConfig.sections[]`, sous-section neuve) : `showInLayerManager: false`, filtré sur les DEUX producteurs du panneau (couches chargées, repeuplement au changement de thème), avant la création de section ; la projection en liste blanche du second chemin porte la clé ; le store n'est pas filtré, la légende le lit. Le sens d'une section déclarée restée vide (titre sans ligne) est relu dans `render-sections.ts`, pas supposé.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **4.3.12** | 22 septembre 2026  | Claude Opus 5.5 | **Deux ajouts publics en 3.7.0, et la quatrième application de la doctrine de forme.** ① `GeoLeaf.GeoJSON.setWorkerUrl` : l'URL de l'ouvrier GeoJSON appartient à l'hôte, pas à un crochet global — relue à chaque construction, survivant à `dispose()`, levant un échec antérieur (§Dépendances et frontières, après le crochet des en-têtes). ② Le bloc `labels` d'une définition de couche est LU, par un résolveur unique (`kernel/geojson/layer-labels.ts`) que tous les lecteurs interrogent ; écrire dans `currentStyle` reste hors contrat (§Contrats). 🖐 Re-vérification du **delta** de ces deux ajouts, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **4.3.12** | 21 septembre 2026  | Claude Opus 5   | **Trois correctifs de caméra dans `kernel/basemaps/` et l'adapter, et la fiche n'en décrivait aucun — vérifié, pas supposé.** Le delta de `45e881671` : le report d'activation d'un fond passe de `map.once("idle")` à l'union `styledata` + `sourcedata` + `idle` ; `terrain.default3D` pose son inclinaison par `jumpTo` et non `easeTo` ; `MaplibreAdapter.fitBounds` transporte l'angle courant. ✅ **Ce qui a été relu et jugé INTACT contre le code** : les quatre seules mentions du sujet ici sont génériques et restent vraies — K-05 nomme la capacité et son événement `geoleaf:basemap:change`, que rien ne touche ; la table des sous-systèmes nomme `basemaps/` et sa façade `GeoLeaf.Baselayers`, dont la surface ne bouge pas ; l'exemple de config et l'entrée « Terrarium » du glossaire sont inchangés. **Aucun `index.ts` n'est touché**, donc rien de ce qui passe la frontière ne bouge ; `global.d.ts` non touché, donc namespace public et §Typage intacts ; le §Contrats **dérive** des `exports` et n'énumère aucun membre. **ADR-01 tient et a été re-vérifié** : le correctif de `fitBounds` vit DANS `adapters/maplibre/`, et le `jumpTo` ajouté à `NativeMap` est une vue structurelle interne à `kernel/basemaps/`, pas un import de valeur de `maplibre-gl`. ⚠️ **La §Séquence de boot ne décrit ni le dévoilement de la caméra ni le recadrage qui la redressait** — c'est précisément pourquoi le défaut a vécu : `init-reveal.ts` recadre ~120 ms après la levée du voile et coupait l'inclinaison du terrain. Rien n'est écrit ici à corriger, et **rien n'y est ajouté** : la phase B nomme les modules, pas les commandes de caméra qu'ils émettent. Le noter plutôt que l'inventer. 🖐 Re-vérification du **delta** de `45e881671`, pas balayage de la fiche.                                                                                                                                                  |
| **4.3.11** | 20 septembre 2026  | Claude Opus 5   | **Troisième application de la doctrine d'irréversibilité, au §Contrats, et elle sert DEUX demandes d'aval par le même geste.** `Layers.getVisibilitySource` rend la source elle-même — `isUserOverridden` ne dit que « pas l'utilisateur », ce qui rend un thème indiscernable d'un seuil de zoom — et rend `null` plutôt qu'une source par défaut sur une couche inconnue, tout nom de source affirmant que quelqu'un a décidé. `Layers.getStyle` rend une projection à deux champs là où l'aval demandait de typer `GeoJSON.getLayerById` : `currentStyle` n'a pas une forme mais **deux** selon la main qui a écrit en dernier, et typer l'entrée aurait gravé cette instabilité sans retour. ✅ **Ce qui a été relu et jugé INTACT contre le code** : la fiche n'énumère aucun membre de `Layers` hors du récit du commit de septembre (§90-91), donc les deux membres neufs n'y font pas trou ; et le point 📌 de la 4.3.5 — rien ici ne décrit `UIConfig` ni les clés `ui.*` — reste vrai, `ui.syncDocumentLang` étant documentée au guide intégrateur et à l'inventaire. 🖐 Re-vérification du **delta** de `50ee43597`, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **4.3.10** | 18 septembre 2026  | Claude Opus 5   | **Le crochet d'en-têtes de l'ouvrier GeoJSON accepte une promesse.** ✅ **Ce qui est entré** (§La frontière core → plugins) : le seul crochet qu'un plugin pose sur le global pour le kernel, sa poignée de main `{ acceptsPromise: true }`, et les trois garanties du poste différé — entrée toujours en attente, sinon rejetée ; rejet ou exception du crochet → envoi sans en-têtes ; poste qui échoue → chargement rejeté. Motif : un fournisseur de jeton asynchrone ne pouvait pas atteindre l'ouvrier, et une couche GeoJSON chargée par URL partait sans jeton. 🖐 Relecture du **delta**, pas balayage de la fiche.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **4.3.4**  | 5 septembre 2026   | Claude Opus 5   | **Re-vérification du commit qui solde l'écriture de visibilité, et UNE phrase du §Contrats a dû bouger.** 🖐 C'est la ligne du tableau documentaire qu'aucun outil ne tient — la VÉRACITÉ : « ce qui est entré au contrat, ce sont **deux** accesseurs nommés » était vrai le 04/09 et se lit au présent ; l'état porte désormais trois lectures et une écriture. Aucune gate ne pouvait le voir (`@param` juste, lien vivant, exemple compilable), et il a été trouvé en confrontant la phrase au commit plutôt qu'en relisant les tags. ✅ **La doctrine n'est pas contredite, elle est APPLIQUÉE une seconde fois** : source obligatoire pour ne pas dupliquer `showLayer`, union de types plutôt qu'enum parce que la surface est type-pure — la contrainte a rendu le résultat plus petit que le geste évident. L'objet à six champs n'est toujours pas public. ✅ **Le reste balayé et intact** : le §Contrats dérive sa liste et aucun fichier de contrat n'est né ; aucun `index.ts` touché, donc la table des sous-systèmes tient ; `global.d.ts` n'est pas touché — la déclaration `Layers?: LayerDataApi` suffit —, donc le §Typage et le §Contrat exposé tiennent. 🖐 Re-vérification du **delta**, pas balayage des 1104 lignes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **4.3.3**  | 5 septembre 2026   | Claude Opus 5   | **Re-vérification du commit qui solde la création de couche à charge fournie** (`inlineData`). ⚠️ Même piège structurel qu'en 4.3.2, et il se reproduira à chaque lot touchant `packages/core/src/` : l'attestation ne peut pas nommer le commit qu'elle accompagne, donc elle se pose au commit SUIVANT. ✅ **Ce qui a été relu** : le §Contrats **dérive** sa liste de la map `exports` et le nombre de fichiers de contrat n'a pas changé — une clé ajoutée à `LayerDefinition` n'y fait donc pas trou. Sa doctrine, elle, a été APPLIQUÉE plutôt que contredite : la question « quelle forme accepte-t-on de ne jamais pouvoir reprendre ? » a été tranchée en restreignant la clé au runtime, et la restriction est mécanique (`layers.schema.json` est `additionalProperties: false`) et non documentaire. La table des 13 sous-systèmes est intacte — aucun `index.ts` touché, donc rien de ce qui passe la frontière ne bouge ; `global.d.ts` n'est pas touché, donc le §Typage et le §Contrat exposé tiennent. 📌 **Rien dans cette fiche ne décrivait la résolution de source du chargeur** (vérifié au grep sur `dataFile` / « sans URL ») : la relaxation du garde-fou ne périme donc aucune phrase. 🖐 Comme en 4.3.2, re-vérification du **delta** et non balayage des 1104 lignes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **4.3.2**  | 5 septembre 2026   | Claude Opus 5   | **Re-vérification d'UN commit — R6, « écrire par diff ».** `b0edff5af` touche 12 fichiers de `packages/core/src/` (+679/−83) **et** cette fiche (+16/−3) dans le même commit. ⚠️ **Une session ne peut pas écrire le sha qu'elle est en train de créer**, donc l'attestation reste sur le sha précédent et `SPECS-FRESH` rougit aussitôt après : tout lot qui touche à la fois un sujet et sa fiche laisse ce rouge derrière lui. Ce n'est pas une négligence, c'est une propriété de la gate, et le remède est un commit de suivi — c'est la même forme qu'en 4.3.1. ✅ **Ce qui a été relu, et contre quoi** : l'édition de R6 au §K-06 dit que l'invariant « le jeu de sous-couches est décidé une seule fois » tient toujours mais que sa PREUVE a changé. Vérifié dans le code, pas sur parole — `applyDataDiff` (`maplibre-adapter.ts`) délègue à `GeoJSONSource.updateData`, qui ne touche que les features d'une source et jamais les couches ; `core.ts:113` n'emprunte le diff que si la couche est adressable **et** si l'adapter rend `true`, sinon repli ligne 116 sur `setData`. Aucun des deux chemins ne rejoue le jeu de sous-couches : l'invariant tient, et par la raison que R6 avance. ✅ **Ce qui a été balayé et jugé intact** : le §Contrats **dérive** sa liste des `exports` et n'énumère aucun membre, donc `applyDataDiff` n'y fait pas trou ; la table des 13 sous-systèmes est inchangée car R6 n'a touché **aucun `index.ts`**, donc rien de ce qui passe la frontière ne bouge ; `global.d.ts` n'est pas touché, donc le namespace public et le §Typage sont intacts ; les trois mentions d'`IMapAdapter` (§Périmètre, ADR-01, glossaire) sont génériques et l'ajout les renforce. 🖐 **Ce qui n'a PAS été fait, et se dit plutôt que se suppose** : aucun balayage phrase à phrase des 1104 lignes. La re-vérification porte sur le **delta** de `b0edff5af`, sur le modèle de la 4.3.1 — pas sur celui de la 4.3.0. |
