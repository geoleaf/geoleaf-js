...
⚠️ **La clé s'appelle `searchable` et non `search`, pour un motif.** `geojson-filter.ts` lit encore
un `search.enabled` que le schéma a retiré : une clé `search` aurait rallumé ce chemin en silence.
