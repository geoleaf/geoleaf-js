...
| Suppression serveur | `serverWinsPreserveLocal` | Le push ne recrée jamais l'entité ; la saisie locale passe en **quarantaine** avec son motif, et n'est jamais détruite sans que l'opérateur l'ait vue. ⚠️ _Annoté le 25/09/2026_ : cette cellule disait « l'entité quitte la carte » et « jamais détruite ». L'entité reste sur l'appareil jusqu'à ce que l'opérateur détruise la saisie ; la destruction retire alors aussi son enregistrement local. La destruction confirmée existe depuis le 07/08/2026 (`ServerDeletionPolicy`, `contracts/sync.contract.ts`). |
