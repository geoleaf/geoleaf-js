...
| `persist`             | boolean                                 | `true`                       | Mirrors the FeatureCollection to `localStorage`. Writes are debounced (~300 ms) and a pending one is flushed when the page is hidden or unloaded, so a measure finished just before a reload is kept. |
