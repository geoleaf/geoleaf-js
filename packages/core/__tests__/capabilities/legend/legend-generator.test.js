...
// SUBJECT does not import — `legend-generator.ts` cites that module nowhere
// —, and it declared there a `getIconsConfig` member that has existed in NO
// source since 11/07/2026. Its absence is even GUARDED
...
// this file set the name, with nothing linking the two. Double inertia — a
// mock with no addressee, carrying a name with no referent.
//
// What the stub REALLY does, and what its name now says: it plays
// `GeoLeaf.Taxonomy` — the only surface the subject reads (`getIcons`, `getCategories`).
const Taxonomy = vi.hoisted(() => ({
    getIcons: vi.fn(() => ({ showOnMap: true })),
...
describe("legend/legend-generator (Phase 5.21)", () => {
    beforeEach(() => {
        ...
        // IS that surface — no more delegation layer whose name designates
