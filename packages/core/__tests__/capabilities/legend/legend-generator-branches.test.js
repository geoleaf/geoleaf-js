...
// ⚠️ This file set here a `vi.mock` of `kernel/config/config-primitives.js`
// its SUBJECT does not import — `legend-generator.ts` cites that module
// nowhere —, and it declared there a `getIconsConfig` member removed from the
// sources on 11/07/2026. Its absence is GUARDED
// (`guards/extracted-features.guard.test.js`, spec `taxonomy-legacy`), but
// that guard scans `src/` SKIPPING `__tests__`: it proved the absence while
// this file wrote the name, and nothing linked the two.
//
// 📌 The removal repairs NO blindness, and that must be said: the delegation
// already went through `GeoLeaf.Taxonomy.getIcons`, and the suite already
// turned red if that reader was renamed in the subject — measured both ways
// on 20/08/2026. What disappears is a double inertia: a mock with no
// addressee, carrying a name with no referent.
...
describe("legend-generator.ts — branch coverage", () => {
    ...
    describe("icon resolution", () => {
        it("resolves icon from taxonomy subcategory field", () => {
            Taxonomy.getIcons.mockReturnValue({ showOnMap: true });
            Taxonomy.getCategories.mockReturnValue({
                CULTURES: {
                    svgId: "culture-icon",
                    subcategories: {
        ...
        it("a sub-category WITHOUT svgId falls back to the parent category icon", () => {
            ...
            Taxonomy.getCategories.mockReturnValue(categories);
        ...
        it("does not use icons when the icons config disables showOnMap", () => {
            Taxonomy.getIcons.mockReturnValue({ showOnMap: false });
        ...
        // ── (c) The COMPOSITION of the two channels, measured spelling by spelling ──
        //
        // The two channels are disjoint by design, and no test measured what
        // their UNION covers. The blind spot was there: `attributes.category`
        // and `attributes.subCategory` were declared by neither — channel 1
        // ignores the prefix, channel 2 ignored the `Id`-less form. Measured
        // on 15/08/2026: 10 spellings out of 12 resolved.
        //
        // ⚠️ This test must be SEEN RED by removing the two `Id`-less entries
        // from `_ATTRIBUTE_FIELD_KIND` — otherwise nothing tells "repaired"
        // from "never exercised". It is worth its COMPLETENESS: adding a
        // spelling to the code without adding it here would leave it
        // unmeasured, exactly the before state.
        it.each([
            ...
            // The spelling is carried in the expectation: a failure NAMES the
            // case, it does not just say "an icon is missing".
        ...
        it("taxonomy lookup returns null when no rule.when", () => {
            Taxonomy.getIcons.mockReturnValue({ showOnMap: true });
            Taxonomy.getCategories.mockReturnValue({ X: { svgId: "x" } });
