 ...
 * the incoming one via `transformStyle` (since MapLibre v5). These tests lock the
 ...
 *
 * Ownership is handed over as a READER (`() => OWNED`), called when the transform
 * runs. Why that timing matters — a style URL downloads before MapLibre runs the
 * callback — is proven through the adapter in `maplibre-style-transform-ownership.test.ts`.
 */
...
describe("buildGeoLeafStyleTransform", () => {
    ...
    test("reads ownership when the transform RUNS, not when it is built", () => {
        const live = { layerIds: new Set(), sourceIds: new Set() };
        // Like the adapter's reader: a fresh copy of what is owned at the moment of the call.
        // Handing back the live sets themselves would hide a read made too early.
        const transform = buildGeoLeafStyleTransform(() => ({
            layerIds: new Set(live.layerIds),
            sourceIds: new Set(live.sourceIds),
        }));
        // A layer created while the style downloads: owned only after the build.
        live.layerIds.add("gl-cities-circle");
        live.sourceIds.add("gl-src-cities");
        const merged = transform(previousStyle(), nextStyle());
        expect(merged.layers.map((l) => l.id)).toEqual(["new-basemap-water", "gl-cities-circle"]);
        expect(merged.sources["gl-src-cities"]).toBeDefined();
    });
